...
The repository already streams or polls market sources into process-local snapshots, computes the internal-inventory route and formula price in memory, supports Redis-backed distributed controls, persists audit evidence to PostgreSQL, and exports API latency histograms. It does not yet provide a distributed precomputed graph router or a database-free production signing path. In production mode, quote creation still performs synchronous durable state transitions, risk evidence reads, treasury/indexer checks, exposure reservation, and remote signing. A local in-memory benchmark therefore cannot prove the production SLO.
...
2. The hot durable tier uses PostgreSQL for principals, API keys, policy/configuration, idempotency, quote lifecycle, risk decisions, exposure reservations, and settlement ownership. These records are authorization and recovery boundaries, not market-data query inputs. Existing synchronous reservation and signer evidence remain mandatory until a separately reviewed replicated reservation ledger plus durable WAL/outbox can preserve the same fail-closed semantics without a database round trip.
...
Track pricing hit/miss rates, stage p99, event-loop saturation, replica concurrency, and signer throughput together. Prefetch and version market/route state, use sticky routing only when it does not weaken availability, and scale API replicas before queueing dominates p50. Keep TTL below the shortest source-state validity window. Any proposal to remove synchronous exposure reservation or signer audit must specify atomic ownership, crash recovery, replay, reconciliation, and fail-closed behavior in a new ADR and production fault test.
