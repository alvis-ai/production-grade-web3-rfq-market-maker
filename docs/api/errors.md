...
- CORS preflight origin 不在 `RFQ_CORS_ALLOWED_ORIGINS` 时返回结构化 `INVALID_REQUEST` 和 HTTP 403，且不返回 `access-control-allow-origin`。
- 未匹配路由或不支持的方法必须返回结构化 `INVALID_REQUEST` 和 HTTP 404，不能返回 Fastify 默认错误对象。

## Rate Limit Policy

The current gateway uses a 60 second rate limit window keyed by the direct client IP by default. `x-forwarded-for` is ignored unless `RFQ_TRUST_PROXY=true`; only enable that setting behind a trusted proxy or ingress that strips spoofed forwarding headers and writes the canonical client address. When proxy trust is enabled, forwarded client identities longer than 128 characters are rejected as `INVALID_REQUEST`/400 before rate-limit buckets are written. Production deployments should replace the in-memory store with Redis or another shared limiter, but the public HTTP contract must stay stable.
