...
| Unroutable aggregated CEX depth | Reference venues inflate `liquidityUsd`, understate size impact, and authorize a quote the deployed hedge worker cannot cover | Explicit `hedge`/`reference` source roles; all healthy sources validate price, only accepted Binance hedge sources contribute depth, and a reference-only quorum fails closed |
