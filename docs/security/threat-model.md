...
| Oversized or stalled remote signer response | A compromised signer, sidecar or proxy sends an unbounded body or only response headers, exhausting API memory or holding risk-approved quote requests open | One full-response AbortController deadline, 1 KiB streaming pre-decode cap, cancellation of unused non-success bodies, generic unavailable errors and independent recovered-signer verification |
| Reused or tampered signer authorization | A compromised gateway references an approved quote id but changes amount, deadline, nonce, pricing attribution, principal or idempotency ownership before requesting KMS signing | Redis persists an exact SHA-256 signing commitment during risk authorization; the isolated signer verifies it before key access and the terminal Lua transaction verifies it again before atomic audit/finalization |
| Partial signer audit and quote finalization | A crash leaves an executable signature without audit, idempotency or nonce state, or transport failure releases exposure after commit | One hash-tagged Redis Lua transaction commits signer audit plus terminal quote state; the API verifies finalization evidence and recovers lost responses conservatively |
...
| Redis exposure split authority | A partitioned rollout admits quotes through Redis while another replica falls back to PostgreSQL, oversubscribing user, pair or Treasury limits | Production requires one `redis-stream` authority, explicit epoch, no automatic PostgreSQL fallback, fail-closed readiness and rollout-wide configuration checks |
| Redis exposure data loss or stale/partial hot state | Lost aggregates, an inventory refresh gap, or a partially refreshed Treasury generation understates signed liability | AOF, replica `WAIT`, bounded backlog, immutable inventory/valuation/Treasury generations, all-target atomic publication, startup warmup, stale-state rejection, expiry grace longer than Treasury max age, stopped-admission epoch recovery and signer-audit reconciliation |
...
| Stale market data, single-venue or demonstration prices | Mispriced production quote from stale evidence, one compromised venue or an unobserved USD-reference basis move | snapshot TTL, market data health check, non-local CEX quorum floor of two, mandatory independent reference exchange, non-local static-provider live-book requirement, conservative Chainlink fallback only |
...
| Unroutable aggregated CEX depth | Reference venues or an unconfigured Binance symbol inflate `liquidityUsd`, understate size impact, and authorize a quote the deployed hedge worker cannot cover | Explicit `hedge`/`reference` roles; production requires at least two sources and an independent reference exchange; only accepted hedge sources contribute depth; API and worker share one route table; startup binds every hedge source to the exact chain, token pair, venue and symbol; a reference-only quorum fails closed |
| Oversized, silent or regressed CEX market data | An exchange, intermediary or compromised connection sends an oversized frame/snapshot, rolls a synchronized local book backward, or leaves a quiet connection indistinguishable from a dead source | Bound WebSocket text frames to 1 MiB and streamed REST snapshots to 2 MiB before JSON decoding; reject binary frames and event-time regression; use post-snapshot Coinbase heartbeat evidence with monotonic time/sequence/trade-id; clear the book, require a new full snapshot and reconnect with capped equal jitter on invalid evidence |
...
| Oversized or stalled CEX execution response | An exchange, intermediary or compromised endpoint returns an unbounded order, trade-history, clock or symbol-rules body, or sends headers and then stalls the body beyond the worker lease | Keep required body reads inside the request AbortController deadline and a 2 MiB pre-decode cap, cancel unused non-success bodies, read signed responses once, and preserve fail-closed domain errors |
...
| Oversized or stalled SDK response | A compromised API, BFF or proxy returns an unbounded JSON/metrics body or only headers, exhausting browser memory or holding quote, submit and lifecycle requests open | One SDK full-response deadline, streaming pre-decode byte cap, declared-length preflight, AbortSignal propagation, body cancellation and generic transport errors |
| Unbounded or drifting PnL history traversal | A large tenant exhausts database/API memory, offset pages skip or duplicate rows during attribution backfill, or a cursor is mistaken for authorization | Default 50/max 100 keyset pages, `limit + 1` detail query, database-enforced immutable creation-time cutoff, deterministic `(realized_at, id)` index, database-side aggregates, closed cursor decoding and principal filtering on every page |
...
| USD-reference token depeg or forged peg evidence | Quotes treat unstable collateral as one USD or consume a wrong/stale oracle round | Dedicated token/USD feed per managed reference token, startup coverage and symbol checks, RPC chain identity, Aggregator metadata, complete fresh rounds, L2 sequencer gate, explicit answer bounds, bounded peg deviation, risk-decision round digest, readiness-driven bounded health metrics and fail-closed signer boundary |
| Daily realized loss exceeds approved appetite or accounting evidence is hidden | Continued signing compounds a loss after hedge execution, fees or dust already consumed the UTC-day budget | PostgreSQL-backed completed hedge-net aggregation by UTC day and configured USD-reference token, exact decimal arithmetic, startup policy coverage, threshold rejection before signer, accounting-window digest, readiness checks, bounded metrics and fail-closed store/error handling |
