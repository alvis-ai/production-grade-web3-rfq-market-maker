...
| Oversized CEX execution response | An exchange, intermediary or compromised endpoint returns an unbounded order, trade-history, clock or symbol-rules body and exhausts worker memory before validation | Stream every Binance HTTP body through a 2 MiB pre-decode cap, reject oversized declared and actual lengths, read signed responses once, and preserve fail-closed domain errors |
