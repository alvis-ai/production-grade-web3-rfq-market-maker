...
- Give the toxic-flow analyzer a dedicated non-interactive principal/key with only `admin:write`; use a separate `admin:read` credential for diagnostics. Every score publication must use `expectedVersion`, and the authenticated actor, analyzer policy version, sample window and observation time must remain in `toxic_flow_score_audit`. Never place either credential in browser or ordinary trading clients.
