...
- `risk_decisions.policy_version` 用于解释风控变更后的历史行为，必须是非空字符串；`reason_code` 只允许出现在 rejected decision 上，approved decision 必须保持 NULL，且 rejected reason 必须来自后端 `RiskRejectReasonCode` 稳定枚举，包括市场流动性不足、波动率越界、USD 单笔名义金额超限、活动用户/交易对累计名义金额超限、缺少可信 USD reference 和确认脱锚的 fail-closed 决策。USD-reference guard 将被检查的 chain、token、aggregator 和 round identity 摘要写入 policy version，原始 answer 不进入公共 API。
