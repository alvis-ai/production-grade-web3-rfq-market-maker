...
- `RFQSettlement.t.sol` includes focused fuzz tests for bounded valid settlement amounts and `amountOut < minAmountOut` rejection. The valid fuzz path signs random bounded quote amounts/nonces and asserts exact token deltas plus nonce consumption; the rejection fuzz path asserts `AmountOutBelowMinimum` leaves nonce and balances unchanged.
