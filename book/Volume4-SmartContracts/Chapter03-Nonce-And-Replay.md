...
Quote response 返回 `nonce` 字符串。Submit request 必须包含同一 nonce。后端不得在 submit 时替换 nonce。

## Engineering Decisions

- 第一版使用 per-user nonce mapping。
- deadline 和 nonce 同时使用。
- nonce 只在成功验证后标记。
...
测试首次提交成功、重复提交失败、不同用户相同 nonce、过期 quote 不标记 nonce 和 transfer revert 回滚 nonce。
