...
当前仓库把 CI 分为 backend、analytics、contract、docs 和 frontend E2E 五条独立门禁。各 workflow 都递归拉取 submodule，避免 ABI 或结构检查在干净 runner 上因缺少锁定的 OpenZeppelin 源码而失败；Backend CI 执行统一的 `make verify`、PostgreSQL quote exposure concurrency，以及 settlement-to-Binance-fill hedge worker E2E；Analytics CI 使用 disposable dependencies 验证 `PostgreSQL -> Redpanda -> ClickHouse` 的真实事务 outbox 链路，在启动 Redpanda/ClickHouse 前单独编译 backend，随后复用同一产物把 migration 与 E2E 拆成分别限时两分钟和五分钟的具名步骤。Migration CLI 在该 job 中以 `RFQ_MIGRATION_TIMEOUT_MS=60000` 收紧内部 deadline，并输出 `connecting`、`acquiring-lock`、`reading-state`、具体版本和 `closing-pool` 阶段；配合请求级、集成进程级、完整脚本级和十分钟 job 级 deadline，外部依赖不会无界占用 runner，worker 也会被有界回收。Contract CI 执行 Foundry 构建/测试、确定性目标链部署验证、临时 Anvil factory-to-receipt settlement E2E，以及把 PostgreSQL callback/indexer、production hedge/fee worker、wallet-only recovery 和 reorg 组合在一起的 core-flow E2E，Docs CI 校验跨代码、配置、部署清单和文档的一致性，Frontend E2E 使用真实 Chromium 与本地 Fastify runtime 验证 quote-to-settlement 浏览器流程。所有第三方 Action 都固定到完整 commit SHA，并保留审计过的版本注释。
...
- Docs CI 检查必需文档。
- Backend CI 运行 typecheck、tests、PostgreSQL concurrency 和生产 hedge/reconciliation worker E2E。
- Analytics CI 运行当前 schema 的 PostgreSQL transactional outbox、Redpanda publish/consume 和 ClickHouse projection E2E。
- Contract CI 运行 Foundry tests、真实 Anvil receipt-confirmed settlement E2E，以及 PostgreSQL-backed settlement/indexer/hedge/fee/reorg core-flow E2E。
...
- Hedge execution E2E starts from a canonical PostgreSQL settlement, runs production reconciliation、hedge and fee workers through a signature-verifying Binance fixture, and asserts exact inventory、fill、commission and hedge-net PnL state before cleanup.
- Contract core-flow E2E starts at the real Anvil `/submit` boundary, requires the durable queued hedge to survive strict Execution Service validation, runs the same production hedge and fee workers, then proves wallet-only indexing and replacement-chain rollback without deleting terminal CEX evidence.
...
- Contract test、Anvil settlement E2E 或 settlement/hedge/reorg core-flow E2E fails：block merge.
...
本地 `make ci-check` 验证 workflow trigger、最小权限、完整 SHA pin、submodule checkout、Contract CI 的两条 Anvil settlement E2E、Backend CI 的 PostgreSQL hedge worker E2E、SBOM/provenance、受限容器运行时门禁、digest signing、Helm package 和 release manifest。`make hedge-worker-e2e` 只允许 loopback PostgreSQL，并在确认数据库没有其他 due worker jobs 后验证 canonical settlement、reconciliation intent、签名 Binance fill、fee reconciliation、inventory 和 hedge-net PnL 的完整收敛。`make settlement-indexer-e2e` 也只允许显式确认的 disposable loopback database，并把真实 `/submit`、duplicate indexer、同一套 production hedge/fee workers、wallet-only recovery 与 replacement-chain reorg 串成一个核心链路。`make container-runtime-check` 本地构建并在最终用户身份与文件系统限制下探测两个镜像。`make verify` 在本机同时存在 Foundry 与 Anvil 时执行 receipt-confirmed 真实结算门禁；destructive core-flow 门禁由 Contract CI 在 disposable PostgreSQL 上独立执行。统一门禁还会运行 Chainlink L1/L2 target-read fixture 与 staging settlement fixture，验证 feed identity/answer bounds 以及单广播、非重试和链/API 证据编排。`make deployment-check` 验证原始清单不存在 `:latest`，并验证每个 Helm Deployment 的 init container 与 runtime container 共用 digest-aware image helper。fixture 不能替代目标环境的 `make chainlink-integration-check` 或带专用低价值钱包的 `make target-settlement-integration-check`。发布流程仍需在受控 tag 上做一次真实 GHCR canary，随后在 staging 校验 Cosign identity、Helm `--atomic` rollback 和 smoke test。
