...
当前仓库把 CI 分为 backend、contract 和 docs 三条独立门禁。Backend CI 拉取递归 submodule 后执行统一的 `make verify`，避免合约 ABI 检查在干净 runner 上因缺少 OpenZeppelin 源码而失败；Contract CI 执行 Foundry 构建和测试；Docs CI 校验跨代码、配置、部署清单和文档的一致性。所有第三方 Action 都固定到完整 commit SHA，并保留审计过的版本注释。
