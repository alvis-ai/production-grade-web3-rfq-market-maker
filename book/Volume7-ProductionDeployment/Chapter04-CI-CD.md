...
当前仓库把 CI 分为 backend、contract、docs 和 frontend E2E 四条独立门禁。四者都递归拉取 submodule，避免 ABI 或结构检查在干净 runner 上因缺少锁定的 OpenZeppelin 源码而失败；Backend CI 执行统一的 `make verify`，Contract CI 执行 Foundry 构建、测试和临时 Anvil receipt-confirmed settlement E2E，Docs CI 校验跨代码、配置、部署清单和文档的一致性，Frontend E2E 使用真实 Chromium 与本地 Fastify runtime 验证 quote-to-settlement 浏览器流程。所有第三方 Action 都固定到完整 commit SHA，并保留审计过的版本注释。
...
- Contract CI 运行 Foundry tests 和真实 Anvil receipt-confirmed settlement E2E。
...
- Receipt-confirmed settlement E2E deploys contracts to a temporary Anvil chain, broadcasts the user's real `submitQuote`, and verifies backend transaction/event ingestion plus post-trade projections.
...
- Contract test or Anvil settlement E2E fails：block merge.
