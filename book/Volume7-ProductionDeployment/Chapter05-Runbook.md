...
| `RFQPortfolioDeltaSoftLimitBreached` | Inspect `delta_evaluation` pre/post components and snapshot ids, current canonical inventory, active reservations, and hedge backlog. | Pause the affected pair or reduce quote size/TTL; restore inventory through reviewed hedge execution. Do not raise limits before reconciling evidence. | Soft-breach counter stops increasing and replayed gross/net delta remains within reviewed limits. |
| `RFQPortfolioDeltaHardLimitExceeded` | Inspect the rejection reason, candidate `delta_evaluation` inputs, current canonical inventory, active reservations, and hedge backlog. | Keep Signer rejection enabled; pause the affected pair and restore inventory through reviewed hedge execution. | Hard-limit rejections clear and independent replay confirms post-trade gross/net delta is within hard limits. |
