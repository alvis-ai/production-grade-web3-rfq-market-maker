...
5. Run `ReconciliationService.reconcileRemovedSettlementToHedge()` and `ReconciliationService.reconcileRemovedSettlementToPnl()` for the same removed event. PnL and an unsubmitted, unleased queued hedge may be removed; a submission-attempted or terminal CEX hedge must remain, stay in inventory replay after any fill, and be compensated through a separately approved reverse trade if it is no longer desired.
