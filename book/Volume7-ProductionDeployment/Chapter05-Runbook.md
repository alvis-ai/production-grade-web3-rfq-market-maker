...
| `RFQQuoteExposureLedgerBacklogHigh` | Compare Redis stream length, pending consumer-group entries, mirror errors, PostgreSQL health and the configured maximum backlog. | Pause new reserves before capacity is exhausted; keep releases enabled and repair the existing consumer group without deleting events. | Backlog drains below the warning threshold, every event is committed before acknowledgement, and readiness returns healthy. |
| `RFQQuoteExposureLedgerFailure` | Break down bounded ledger failure reasons and mirror errors; verify AOF, replica acknowledgements, epoch, fused lease/read plus commit/unlock behavior and PostgreSQL mirror health. | Keep new quote admission stopped, preserve Redis state and use no PostgreSQL fallback authority; allow only risk-reducing release operations. | Failure counters stop increasing, AOF/replicas and mirror probes pass, and a bounded reserve/release canary is audited end to end. |
| `RFQQuoteExposureLockWaitP99High` | Compare fused lease acquisition/read p99 with quote rate, hot-chain distribution, cleanup work and Redis CPU/network saturation. | Reduce admitted concurrency or pause the hot chain; do not extend lease TTL until fused read, in-memory evaluation and commit/unlock duration are measured. | Lock-wait p99 remains below 25ms for two windows without lease loss, reject spikes or backlog growth. |
| `RFQQuoteExposureMirrorStalled` | Confirm backlog is non-zero, mutations continue and `rfq_quote_exposure_ledger_mirrored_total` has no progress; inspect pending/claimed entries and PostgreSQL errors. | Keep reserves gated, repair the poison event or PostgreSQL dependency, and let the same consumer group retry before any acknowledgement. | Mirrored counters advance, pending entries and backlog drain, projection positions match retained audit events, and readiness recovers. |
| `RFQQuoteIssuanceBacklogHigh` | Compare issuance Stream length, pending entries, oldest age, projector throughput, PostgreSQL saturation and the configured maximum. | Pause new quotes before the hard backlog bound; preserve hot state and all unacknowledged events while restoring the existing consumer group. | Backlog drains below warning and then to zero, with every terminal hot quote represented in PostgreSQL. |
| `RFQQuoteIssuanceFailure` | Break down `backlog_full`, `replica_ack`, `state_invalid` and `projection_timeout`; inspect AOF, replicas, epoch, payload conflicts and migration 038. | Keep signing stopped, preserve the Redis authority and do not start the PostgreSQL compatibility backend; repair durability or the exact poison state under the same epoch. | Failure counters stop, a canary reaches finalized hot state and PostgreSQL projection, and exact replay returns the same response. |
| `RFQQuoteIssuanceMirrorStalled` | Confirm mutations continue while `rfq_quote_issuance_mirrored_total` does not; inspect pending ownership, first source id, projection version and PostgreSQL transaction errors. | Let `XAUTOCLAIM` recover stale entries, repair the sink or conflicting projection and retain the event until commit; never trim or manually ACK it. | Projected counters advance, pending entries clear, backlog returns to zero and exposure projection has no prepared-parent ordering errors. |
...
| `RFQQuoteLatencyP99High` | Compare `rfq_quote_stage_latency_seconds` by stage with API CPU, event-loop saturation and replica queueing. | Shed low-priority flow or pause slow pairs; scale only after PostgreSQL, Redis, RPC and signer headroom is verified. | `rfq_quote_latency_seconds` p99 remains below 50ms for at least two windows. |
| `RFQQuoteStageLatencyP99High` | Identify the `stage` above 25ms and correlate it with the owning dependency's health and error metrics. | Isolate the slow dependency, fail closed where risk state is uncertain, and reduce admitted quote concurrency until the stage recovers. | Every bounded stage remains below 25ms p99 for at least two windows. |
| `RFQPricingCacheIneffective` | Compare pricing hit/miss rates, snapshot churn, request distribution and configured TTL/capacity. | Restore stable market snapshots or correct cache configuration; do not widen keys or TTL in ways that can reuse stale inventory or route state. | Pricing cache records hits under sustained repeated flow and quote correctness checks remain green. |
...
| `RFQPortfolioDeltaSoftLimitBreached` | Inspect `delta_evaluation` pre/post components and snapshot ids, identify whether the chain/token asset、gross or signed net soft limit fired, then compare canonical inventory, active reservations and hedge backlog. | Pause the affected pair or reduce quote size/TTL; restore the specific asset inventory through reviewed hedge execution. Do not raise limits before reconciling evidence. | Soft-breach counter stops increasing and replayed asset/gross/net delta remains within reviewed limits. |
| `RFQPortfolioDeltaHardLimitExceeded` | Inspect the rejection reason and candidate delta inputs, identify the chain/token asset、gross or signed net hard breach, then reconcile canonical inventory, active reservations and hedge backlog. | Keep Signer rejection enabled; pause the affected pair and restore inventory through reviewed hedge execution. | Hard-limit rejections clear and independent replay confirms post-trade asset/gross/net delta is within hard limits. |
| `RFQGammaGuardrailTriggered` | Group `GAMMA_GUARDRAIL_TRIGGERED` decisions by policy version and pair, then independently recompute inventory, USD-notional and volatility utilization from the quote snapshot and projected positions. Confirm the exact piecewise buckets and multiplier boundary. | Keep rejection enabled; reduce quote size, restore inventory or pause the pair while volatility remains elevated. Do not raise `maxRiskMultiplierBps` or bucket thresholds merely to restore volume. | Recomputed utilization leaves the rejecting combination, the alert clears, bounded quotes sign, and an exact-threshold negative canary remains rejected. |
| `RFQTreasuryLiquidityInsufficient` | Confirm the `TREASURY_LIQUIDITY_INSUFFICIENT` reason, group rejects by chain and output token using quote audit records, inspect the current hot-state generation age/block, compare unexpired `quote_exposure_reservations.amount_out` with an independently verified same-block Treasury ERC20 balance, and verify custody transfers or settlement outflows. | Pause the affected pair or replenish the reviewed Treasury through the custody procedure. Never widen freshness, bypass the hot-state read, or delete unexpired reservations to manufacture capacity. | A fresh complete generation shows Treasury balance safely above all unexpired output reservations plus the operating buffer, the alert clears, and a canary quote signs without settlement revert. |
...
| `RFQUsdReferenceDepeg` | Group `rfq_usd_reference_health_failures_total{reason="DEPEG"}` by `chain_id,token`, pause every pair using the affected reference token, compare the configured Chainlink proxy round with a second reviewed token/USD source, and verify chain ID, feed description, decimals, sequencer status and policy-version round digest without logging credential-bearing RPC URLs. This alert is readiness-driven and does not require quote traffic. | Keep signing blocked. Follow custody and hedge incident procedures for existing inventory; do not widen `maxDeviationBps`, relabel the token, replace the feed or resume through manual quote-control until the economic peg and oracle evidence independently agree. | Independent sources remain within the reviewed limit for the recovery window, a new oracle round passes metadata/freshness checks, `rfq_usd_reference_health_safe` is 1 on every replica, risk readiness is `ok`, and a bounded canary signs only after an audited pair resume. |
| `RFQUsdReferenceHealthUnsafe` | Read `chain_id,token` from `rfq_usd_reference_health_safe=0`, then group the bounded failure counter by reason. Check RPC identity/reachability, sequencer state, exact feed metadata, round shape/freshness and configured answer bounds according to `RPC_UNAVAILABLE|RPC_CHAIN_MISMATCH|SEQUENCER_UNAVAILABLE|METADATA_MISMATCH|FEED_UNAVAILABLE|ROUND_INVALID|ROUND_STALE|ROUND_FUTURE|ANSWER_OUT_OF_BOUNDS|DEPEG`. | Keep every affected pair fail-closed and restore the exact dependency or reviewed feed configuration. Do not suppress readiness probes, widen freshness/future-skew/bounds, or replace oracle evidence with a static peg. | The gauge returns 1 on every API replica, failure counters stop increasing, independent oracle evidence agrees, and risk readiness remains `ok` through the recovery window. |
| `RFQRiskDependencyUnavailable` | Check `/ready.components.risk`, settlement RPC reachability, configured chain and settlement addresses, Treasury generation age/coverage and refresh-failure logs, Redis exposure-ledger durability/replication, mirror health, inventory/valuation hot-state freshness, and risk policy loading. | Keep quoting fail-closed, route traffic away from unhealthy replicas, and restore the dependency. Do not disable Treasury evidence, bypass the mirror gate, force-publish a partial generation, or substitute stale balances. | Risk readiness is `ok`, a complete Treasury refresh publishes on every replica, Redis admission and PostgreSQL mirroring progress, and valid quotes resume while malformed or over-capacity requests still fail before signing. |
...
| `RFQSignerAuditUnavailable` | Check signer `/ready`, migrations `027`-`028` and `036`, Redis TLS/AOF/replication, PostgreSQL TLS, least-privilege grants and audit capacity without logging credentials, quote digests or signatures. | Keep quote admission failed closed, restore the durable stream and mirror path, and never bypass audit or switch to memory. | Audit errors stop, every signer replica is ready, and a canary signature has one context-version-2 row with the expected epoch-qualified source id. |
| `RFQSignerAuditStreamBacklogHigh` | Compare `rfq_signer_audit_stream_backlog`, mirror errors, PostgreSQL latency, consumer pending entries and stale consumer ownership. | Keep the hard backlog limit unchanged, restore PostgreSQL or consumer progress, and let `XAUTOCLAIM` recover stale entries; never trim or acknowledge unmirrored events. | Backlog drains below the warning threshold, pending age falls, and sampled Redis entries have exactly one PostgreSQL source id. |
| `RFQSignerAuditAdmissionFailures` | Break down append results into `backlog_full` and `replica_ack_failed`; inspect Redis AOF health, replica count, replication lag and the configured maximum backlog. | Keep signing failed closed, restore the required replica or mirror capacity, and do not lower replica acknowledgements or raise backlog merely to resume traffic. | New events receive the required `WAIT` acknowledgements, failure counters stop, and a canary is mirrored before its stream entry is removed. |
| `RFQSignerAuditMirrorErrors` | Inspect PostgreSQL reachability, migration `036`, insert grants, payload-integrity failures, pending consumer entries and the first unacknowledged source id. | Repair the sink or malformed producer rollout while preserving pending entries; do not skip, delete or manually ACK poison evidence. | Mirror errors stop, stale entries are reclaimed, duplicate retries converge through `ON CONFLICT`, and backlog drains. |
| `RFQSignerAuditMirrorStalled` | Confirm backlog is non-zero, compare consumer-group pending age with `rfq_signer_audit_mirrored_total`, and inspect whether the mirror loop is blocked on Redis or PostgreSQL. | Restore the mirror worker or sink dependency without trimming the stream; allow pending ownership recovery and idempotent PostgreSQL replay to resume. | The mirrored counter advances, pending age falls, and backlog returns to zero without missing source ids. |
...
| `RFQHotStateRefreshFailures` | Group by fixed `state`, compare failures with readiness and PostgreSQL/RPC health. Confirm memory capacity when `state="toxic_flow"` and complete configured target coverage for multi-target snapshots. | Keep signing fail-closed. Restore the control-plane database/RPC or correct invalid evidence; do not truncate scores, publish a partial generation or bypass the hot view. | Every replica publishes successful complete generations, the failure counter stops increasing, readiness is healthy and canary quotes show no database/RPC stage latency. |
| `RFQHotStateStale` | Group by fixed `state`, compare the last-success timestamp with refresh duration, event-loop delay and the configured maximum age. | Keep signing fail-closed and restore the blocked refresher; do not widen max age or reuse an expired process-local generation. | Last-success age remains below `RFQ_HOT_STATE_MAX_AGE_MS` on every replica for two refresh windows and bounded canary quotes succeed. |
...
| `RFQDailyLossLimitExceeded` | Compare `rfq_daily_realized_pnl_usd` and `rfq_daily_loss_limit_remaining_usd` for the affected chain/reference token, then reconcile completed hedge-net rows, fills, fees, dust valuation, pricing versions and UTC boundaries. | Keep signing blocked for every managed pair using that reference token. Do not raise the limit or edit accounting rows during the incident; reduce inventory through an approved hedge and deploy a reviewed policy change only after loss attribution is complete. | The next UTC window or an approved policy rollout restores positive remaining budget, `rfq_daily_loss_risk_safe` returns 1, readiness is healthy, and a canary quote records matching accounting-window evidence. |
| `RFQDailyLossRiskUnavailable` | Group `rfq_daily_loss_risk_failures_total` by `chain_id,token,reason`; inspect PostgreSQL reachability, migration 024 hedge-net columns, row types and API read permissions without logging raw rows or credentials. | Keep the affected reference token fail-closed. Restore exact completed hedge-net evidence and never substitute zero, cached PnL, ClickHouse projections or pending rows. | Failure counters stop increasing, a direct evidence read returns an exact current-UTC-day total, `rfq_daily_loss_risk_safe` returns 1 and risk readiness recovers. |
...
Page immediately for `RFQQuoteExposureLedgerFailure`, `RFQQuoteExposureMirrorStalled`, a backlog approaching its configured maximum, or lock-wait p99 consuming the quote tail budget. First pause new quote admission; do not switch individual pods to PostgreSQL and do not clear Redis keys. Confirm AOF status, replica health, the configured epoch, pending consumer-group entries and PostgreSQL migration 037 before restarting consumers.

For a mirror-only outage, the process-local health gate blocks new reserves after the failed event or bounded PostgreSQL probe; keep quote admission stopped and leave risk-reducing releases enabled. Repair PostgreSQL or the poison event, let the same consumer group drain, and verify event count plus latest projection position before unpausing. Never acknowledge or delete an event that was not committed to PostgreSQL.
...
### Quote Issuance Journal

Page immediately when issuance backlog approaches its configured maximum, any durability failure increases, mirror progress stalls, or submit projection waits time out. Stop new quote admission without disabling status, submit or post-trade recovery. Record the active epoch, Stream length, pending entries, consumers, oldest idle time, latest PostgreSQL projection positions and Redis AOF/replica state before restarting anything.

The Redis hot state is the active idempotency/nonce/authorization authority. Do not delete keys, trim the Stream, lower replica acknowledgements, raise the backlog to hide a stalled projector, or start PostgreSQL issuance on selected replicas. Repair PostgreSQL migration 038, pool capacity or the first poison event, then let the same group replay cumulative events. A later cumulative stage may reconstruct missing earlier stages, but exact hash or identity conflicts require reviewed reconciliation rather than manual success.

Exposure projection waits for the Redis `prepared` projection marker before writing its PostgreSQL foreign-key child. Repeated exposure mirror foreign-key errors after ADR-0021 indicate a mixed-version rollout or missing barrier and require admission to remain paused. Recovery requires issuance and exposure backlog zero, no new mirror errors, one ordered event chain per sampled quote, terminal idempotency agreement and a successful exact replay plus submit canary.

### Quote Issuance Persistence

When `quote_preparation_persistence`, `authorization_persistence` or `issuance_persistence` p99 rises, pause benchmark traffic and separate Redis command latency, signer latency, event-loop saturation and projector load. Production issuance mutations do not wait for PostgreSQL; synchronous SQL in these stages indicates an invalid backend selection. Repeated exposure foreign-key failures indicate a missing prepared-projection barrier or mixed-version rollout. Do not disable foreign keys or exact state guards. Verify each affected quote has one hot identity, one snapshot, one route, one risk decision, matching idempotency ownership, at most one signed payload and an ordered PostgreSQL projection before resuming.
...
Run game days: signer unavailable, stale market data, indexer lag, Redis AOF/replica loss, PostgreSQL exposure-mirror outage and recovery, hedge venue failure, and denied approved/unapproved FQDN egress. Verify alert, automatic reserve gate, risk-reducing release, action and recovery.
