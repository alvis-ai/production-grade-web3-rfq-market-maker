...
| `RFQUsdReferenceDepeg` | Confirm `USD_REFERENCE_DEPEG`, pause every pair using the affected reference token, compare the configured Chainlink proxy round with a second reviewed token/USD source, and verify chain ID, feed description, decimals, sequencer status and policy-version round digest without logging credential-bearing RPC URLs. | Keep signing blocked. Follow custody and hedge incident procedures for existing inventory; do not widen `maxDeviationBps`, relabel the token, replace the feed or resume through manual quote-control until the economic peg and oracle evidence independently agree. | Independent sources remain within the reviewed limit for the recovery window, a new oracle round passes metadata/freshness checks, risk readiness is `ok`, and a bounded canary signs only after an audited pair resume. |
