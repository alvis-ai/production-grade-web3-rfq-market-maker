...
- `rfq_quote_stage_latency_seconds` with a bounded `stage` label; API admission exposes `rate_limit` and `quote_control`; production Redis direct admission records the combined preparation/authorization transaction as `authorization_persistence` and should have zero `quote_preparation_persistence` samples, while PostgreSQL and custom compatibility stores retain separate preparation/authorization stages; alert when any active stage p99 remains above 25ms
