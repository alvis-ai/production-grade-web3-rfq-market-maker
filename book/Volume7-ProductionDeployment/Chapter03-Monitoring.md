...
- `rfq_quote_stage_latency_seconds` with a bounded `stage` label; API admission exposes `rate_limit` and `quote_control`; production same-slot Redis records the combined exposure/preparation/authorization transaction as `quote_admission` and should have zero successful `exposure_reservation`, `quote_preparation_persistence`, and `authorization_persistence` samples, while compatibility stores retain separate stages; alert when any active stage p99 remains above 25ms
- `rfq_signer_service_stage_latency_seconds{stage="authorization"}` must remain below `RFQ_SIGNER_AUTHORIZATION_WAIT_MS`; any signer authorization error or sustained wait near the bound means the gateway/signer capability rollout, Redis authority, or admission ordering is inconsistent. Production readiness must advertise both `atomic_quote_commit_v1` and `durable_authorization_wait_v1` before overlap is enabled.
...
- `rfq_quote_exposure_ledger_version_conflicts_total`; correlate conflict rate with successful reserve rate
- `rfq_quote_exposure_ledger_lock_wait_seconds`; legacy metric name covering complete post-contention CAS or release coordination, with p99 kept inside the quote-stage budget
...
- Quote exposure alerting correlates `rfq_quote_exposure_ledger_version_conflicts_total`, post-contention `rfq_quote_exposure_ledger_lock_wait_seconds`, Lua-returned backlog, bounded failures and mirror errors. Sustained conflicts above four per successful reserve indicate CAS churn or starvation even before the retry budget fails closed. A mirror failure is an admission-health failure even while Redis remains reachable; keep releases enabled, repair and drain the same consumer group, and never create a PostgreSQL fallback authority.
