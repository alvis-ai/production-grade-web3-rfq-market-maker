...
- Every API, hedge, analytics, reconciliation, and settlement-indexer rollout runs the same migration init container. Migration 005 (`005-post-trade-reconciliation.sql`) creates the quote-scoped revision queue and replaces permanent quote uniqueness with one-canonical-settlement uniqueness. Migration 006 (`006-quote-snapshot-pnl.sql`) archives legacy simulated PnL and installs snapshot-bound attribution. Migration 007 (`007-settlement-indexer.sql`) adds leased chain cursors, block-hash checkpoints, and the canonical chain/block index before the indexer becomes ready.
