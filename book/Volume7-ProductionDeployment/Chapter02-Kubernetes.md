...
- `rfq-signer-secrets` is mounted only by the signer workload and contains `RFQ_AWS_KMS_KEY_ID`, the matching internal service token, `RFQ_TRUSTED_SIGNER_ADDRESS`, `RFQ_SETTLEMENT_ADDRESS`, signer TLS certificate/private key, a least-privilege audit PostgreSQL URL and the TLS Redis URL used for durable stream admission. In production an external secret controller should materialize the same token into the separate API and signer Secrets without granting either workload read access to the other's Secret.
...
- Migration init containers serialize DDL with one PostgreSQL session advisory lock. Contenders poll for at most 240 seconds so ordinary multi-pod rollouts wait for the active migrator, while a wedged owner eventually causes an observable restart instead of unbounded startup. The CLI emits its active stage, closes the pool on every settled success or error path, and exits after the default 300-second hard deadline; operators may set `RFQ_MIGRATION_TIMEOUT_MS` between 1000 and 1800000 ms, but it must exceed the reviewed worst-case migration time and remain below the init-container restart budget.
...
- `RFQ_RISK_POLICY_JSON` is versioned non-secret policy. Each token limit is chain-scoped and uses raw-unit strings for max input、minimum output and absolute inventory plus an integer-dollar `maxNotionalUsd`; global `minLiquidityUsd` and `maxVolatilityBps` reject unsafe market regimes before signing. `portfolioVar` versions the aggregate USD budget, confidence multiplier, horizon, snapshot freshness and every non-USD token's explicit USD-reference valuation pair. `portfolioDelta.assetLimits` must cover those valuation assets exactly with chain/token soft-hard USD limits, while separate gross/net limits cap aggregate direction. Required `gammaGuardrail` thresholds projected inventory、USD notional and volatility utilization, then rejects a reviewed piecewise multiplier boundary before signing. Runtime takes the smaller token-side notional limit and compares USD-reference token base units using registry decimals. Startup cross-checks every policy token against the registry, every managed pair against both token limits, and requires matching VaR valuation plus delta asset limits for every non-USD risk token. Registry and risk policy changes must ship atomically in one rollout; a partial update intentionally fails startup/readiness instead of silently widening access. Because `token-limit-risk-v2` is a strict-schema break, first apply migration 033, pause quote creation, drain one maximum quote TTL, then replace API and Signer workloads with the same image and policy revision before resuming. Old binaries reject `gammaGuardrail`, while new binaries reject its absence; do not rely on an ordinary mixed-version rolling update.
...
- Helm `signerSecret` references only the API-side service token、CA、trusted signer and settlement address. `signerService.secret` references the isolated KMS/TLS material plus signer-owned audit PostgreSQL and Redis URLs, while `signerService.serviceAccount.annotations` carries workload identity. API `serviceAccount.annotations` stays empty.
...
- API, hedge worker, analytics worker, reconciliation worker, settlement indexer and toxic-flow analyzer Deployments run `backend/dist/db/migrate.js` in an init container before application startup. The init container alone receives `rfq-database-migration-secrets`; runtime containers retain lower-privilege database roles without DDL rights. The migration runner holds a PostgreSQL session advisory lock across discovery and every pending DDL transaction, so concurrent rollout replicas serialize the full migration chain through `038` instead of racing `_migrations` state. Migration 027 creates isolated append-only signer evidence, migration 028 binds every new signing audit row to risk authorization context, migrations 029-035 extend durable risk and execution evidence, migration 036 makes signer stream replay idempotent, migration 037 installs the quote-exposure projection, and migration 038 installs the cumulative quote-issuance journal plus ordered projection version. Reconciliation and toxic-flow workers receive the same reviewed `RFQ_TOKEN_REGISTRY_JSON` as the API because PnL and execution-price normalization must use identical token decimals.
- Production API pods require `RFQ_QUOTE_EXPOSURE_BACKEND=redis-stream`, `rediss://`, AOF health, at least one replica acknowledgement and a pre-created `RFQ_QUOTE_EXPOSURE_LEDGER_EPOCH`. Runtime epoch initialization remains disabled. Before first rollout, stop quote admission, set the reviewed URL/prefix/epoch plus `RFQ_QUOTE_EXPOSURE_BOOTSTRAP_CONFIRM=initialize-empty-quote-exposure-ledger`, run `make quote-exposure-ledger-bootstrap` from approved operations egress, verify migration 037 and signer-audit coverage, then deploy all API replicas with the same epoch. The command refuses any non-empty prefix. Pod identity is included in the consumer id so a shared consumer group can reclaim abandoned stream entries without collisions; `RFQ_QUOTE_EXPOSURE_MIRROR_CLEANUP_INTERVAL_MS` bounds each pod's PostgreSQL health/expiry probe frequency.
- Production API pods also require `RFQ_QUOTE_ISSUANCE_BACKEND=redis-stream`, a separately hash-tagged prefix, healthy AOF, at least one replica acknowledgement, a reviewed `RFQ_QUOTE_ISSUANCE_LEDGER_EPOCH` and disabled runtime epoch initialization. All replicas share the consumer group but use unique consumer ids. Keep `RFQ_QUOTE_ISSUANCE_MAX_BACKLOG` below the capacity where PostgreSQL catch-up violates retention, and keep hot-state/idempotency TTLs longer than the submit projection wait plus operational recovery window. Never trim unacknowledged issuance entries or start the PostgreSQL compatibility backend beside the Redis authority. Exposure projection waits for the matching prepared issuance marker, so deploy migration 038 and start the issuance projector before accepting quotes.
...
- `NODE_ENV=production` requires API `RFQ_SIGNER_MODE=remote` with an HTTPS signer URL, service token, trusted signer, settlement address and mounted CA. The isolated signer requires `RFQ_SIGNER_MODE=aws-kms`, KMS identity, TLS certificate/key, the shared token/risk policy and `redis-stream` audit mode with AOF health, at least one replica acknowledgement and an explicit stream epoch. Raw private keys, production HTTP signer origins, plaintext Redis and zero-replica audit admission are rejected. Signer `/ready` performs KMS sign+verify with 30-second coalesced caching and verifies audit-stream health; API `/ready` probes that endpoint without creating a separate static quote signature.
