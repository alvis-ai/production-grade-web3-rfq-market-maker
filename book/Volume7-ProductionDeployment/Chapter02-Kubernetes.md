...
- Helm `hedgeWorker.secret` references the isolated credential Secret, while shared `env.RFQ_HEDGE_ROUTES_JSON` contains non-secret route, decimals, raw step-size, price-tick and maximum-slippage metadata for both API and worker. The Binance key needs only `TRADE` and `USER_DATA`, must be IP restricted, and must keep withdrawals disabled because the same worker queries account fills for exact commissions. CEX source、route and global registry values are one rollout unit; a mismatch intentionally prevents API or worker startup.
