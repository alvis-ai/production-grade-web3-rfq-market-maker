...
- `rfq-backend-secrets` Secret for `DATABASE_URL`、`RFQ_AWS_KMS_KEY_ID`、`RFQ_TRUSTED_SIGNER_ADDRESS`、`RFQ_SETTLEMENT_ADDRESS` and `RFQ_REDIS_URL`. It must not contain `RFQ_SIGNER_PRIVATE_KEY`.
...
- `RFQ_RISK_POLICY_JSON` is versioned non-secret policy. Each token limit is chain-scoped and uses raw-unit strings for max input、minimum output and absolute inventory. Startup cross-checks every policy token against the registry and every managed pair against both token limits. Registry and risk policy changes must ship atomically in one rollout; a partial update intentionally fails startup/readiness instead of silently widening access.
...
- Every API, hedge, analytics, and reconciliation rollout runs the same migration init container. Migration 005 creates the quote-scoped revision queue and replaces permanent quote uniqueness with one-canonical-settlement uniqueness. Migration 006 archives legacy simulated PnL, installs snapshot-bound attribution columns and upgrades PnL analytics events before any reconciliation worker becomes ready.
- Helm `signerSecret` references the KMS key id、trusted signer address and settlement address without embedding values into chart templates. `serviceAccount.annotations` carries the workload-identity binding, not AWS credentials.
...
- API, hedge worker, analytics worker and reconciliation worker Deployments run `backend/dist/db/migrate.js` in an init container before application startup. The init container alone receives `rfq-database-migration-secrets`; runtime containers retain lower-privilege database roles without DDL rights. The migration runner holds a PostgreSQL session advisory lock across discovery and all pending DDL transactions, so concurrent rollout replicas serialize through `005-post-trade-reconciliation.sql` and `006-quote-snapshot-pnl.sql` instead of racing `_migrations` state. The reconciliation worker receives the same reviewed `RFQ_TOKEN_REGISTRY_JSON` as the API because rebuilt PnL must use identical token decimals.
