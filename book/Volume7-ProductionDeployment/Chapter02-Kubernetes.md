...
- Every API, hedge, analytics, reconciliation, and settlement-indexer rollout runs the same migration init container. Migration 005 (`005-post-trade-reconciliation.sql`) creates the quote-scoped revision queue and replaces permanent quote uniqueness with one-canonical-settlement uniqueness. Migration 006 (`006-quote-snapshot-pnl.sql`) installs snapshot-bound attribution; Migration 007 (`007-settlement-indexer.sql`) adds leased chain cursors and block-hash checkpoints; Migration 008 (`008-submit-reservations.sql`) adds the shared API submit lease. Migrations 009 and 010 extend durable risk reasons, Migration 011 (`011-open-quote-exposure.sql`) installs TTL-bound cumulative user/pair quote exposure reservations, and Migration 012 (`012-pricing-attribution.sql`) makes volatility premium and hedge cost durable quote components before replicas become ready.
