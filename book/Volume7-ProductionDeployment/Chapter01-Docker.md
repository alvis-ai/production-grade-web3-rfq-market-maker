...
- Frontend image builds static Vite assets and serves them with Nginx. It includes a local-default `/runtime-config.js`; Kubernetes replaces that public file at deployment time, so API URL, settlement address, and WalletConnect project id do not require an environment-specific image rebuild.
