...
Active policy、inventory、valuation snapshots 与 Treasury balances 应在请求外刷新，并带版本、观测时间、全量覆盖和失效机制。生产准入不查询 PostgreSQL，也不在请求中回源 Treasury RPC；Redis chain lease 仍会串行化同链 reservation，因此必须跟踪 fused acquisition/read 的 lock-wait 与 mutation latency。将成功 reserve 从五次 Redis 往返压缩到两次后，两次真实依赖栈并发 5 复测的 exposure 平均从基线 `12.84 ms` 降到 `5.54-6.23 ms`，但整条链路仍未达到 `p50 < 10 ms`：远程签名和同步 lifecycle/risk audit 已成为更主要的剩余成本。
