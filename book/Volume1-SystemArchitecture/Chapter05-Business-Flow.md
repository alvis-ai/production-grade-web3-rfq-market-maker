...
`POST /quote` 返回 signed quote。`POST /submit` 可以作为 relay 或 tx payload 生成入口，并在模拟执行路径返回 `settlementEventId`、`hedgeOrderId` 和 `pnlId`。`GET /quote/:id` 返回 quote 当前状态，`GET /settlements/:id` 返回已消费的 settlement event，`GET /hedges/:id` 返回对冲意图状态，`GET /pnl` 返回 realized spread PnL 汇总。内部事件接口不对用户公开。
