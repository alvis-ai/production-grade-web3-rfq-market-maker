...
Execution Service 处理 quote 提交路径。根据产品模式，它可以生成交易 payload 给用户钱包，也可以作为 relay 提交交易。当前参考实现用同步模拟结算把 `/submit -> inventory update -> hedge intent -> metrics` 链路跑通；生产模式下链上 `RFQSettlement` 事件仍是最终结算权威。
...
- 可选 relay 到链上。
- 追踪 txHash。
- 第一阶段同步应用 settlement delta、更新库存、创建 hedge intent。
