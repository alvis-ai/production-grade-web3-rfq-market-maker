...
- Settlement event ingestion validates `blockNumber` and `logIndex` as non-negative safe integers before building the idempotency key or applying inventory side effects.
...
- PnL idempotency requires the repeated `(quoteId, model)` input to match the stored attribution payload. A retry with a different amount, token pair or quote id is treated as a PnL record conflict rather than silently returning the previous record.
