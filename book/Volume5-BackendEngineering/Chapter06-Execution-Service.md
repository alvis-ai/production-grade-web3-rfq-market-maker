...
- Receipt reader 在读取 transaction 或 receipt 之前调用 `eth_chainId`，并要求它与 quote 和配置中的 chain 完全一致；错链和 RPC 异常统一映射为 `SETTLEMENT_UNAVAILABLE`，且不能触发后续链读取。非本地运行时只接受 HTTPS `RFQ_RECEIPT_CONFIG_JSON.rpcUrl`，开发环境仍允许本地 Anvil 或 Compose 的 HTTP endpoint。相同 RPC 被 Treasury liquidity provider 使用时，首次成功的链身份校验按 provider 生命周期缓存，失败的校验会删除缓存并允许恢复后重试；任何未验证链都不能进入 `treasury()` 或 `balanceOf()` 读取。
