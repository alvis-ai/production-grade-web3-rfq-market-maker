...
- Execution Service 使用与 Quote Service 相同的显式代码边界：`execution.service.ts` 只编排 verify、evidence、event、inventory 和 hedge；`execution-service-contract.ts` 定义并快照构造依赖、provider、planner 与 execution context；`execution-service-result-validation.ts` 隔离 settlement verification/evidence/event store 的不可信返回值；`execution-service-post-trade-validation.ts` 隔离 inventory 返回值并稳定导出 hedge validator；`execution-service-hedge-result-validation.ts` 独立校验 durable hedge envelope。公共调用仍从 `execution.service.ts` 导入，避免为了文件拆分改变 API。`make api-composition-check` 对这些边界分别设置行数上限，并禁止把已拆出的 validator 重新塞回 orchestrator。
...
- Receipt ingestion and the settlement indexer read the canonical block timestamp and persist it as `settlement_events.settled_at`; the public settlement `observedAt` exposes this execution time. Database `created_at` remains ingestion metadata and must not anchor post-trade markout horizons, especially during delayed indexer catch-up. Legacy rows without recoverable block time are not automatically analyzed.
...
- Settlement indexer 在启动 dependency check 和每次 chain poll 都执行 `eth_chainId`。该检查发生在 `claimCursor` 之前，因此错链 endpoint 只能产生稳定的 `RPC_OR_STORE_UNAVAILABLE`，不能领取 lease、读取日志或改变 cursor。非本地 indexer 同样只接受 HTTPS RPC；本地 Compose 的 `NODE_ENV=development` 明确保留 HTTP Anvil 支持。
...
- `/submit` uses `LocalSettlementVerifier` to mirror RFQSettlement signature shape, canonical low-s/v checks, EIP-712 trusted signer recovery, chainId、deadline、token whitelist、token pair 和 amount shape checks before consuming settlement evidence. The verifier reuses the backend signer typed-data builder, so `ProductionGradeRFQ` domain name/version, quote fields, `chainId`, and `settlementAddress` stay aligned with signed quote creation. During rotation it accepts the primary plus at most four explicit overlap addresses, matching the contract's five-signer hard bound.
...
- Receipt reader 在读取 transaction 或 receipt 之前调用 `eth_chainId`，并要求它与 quote 和配置中的 chain 完全一致；错链和 RPC 异常统一映射为 `SETTLEMENT_UNAVAILABLE`，且不能触发后续链读取。非本地运行时只接受 HTTPS `RFQ_RECEIPT_CONFIG_JSON.rpcUrl`，开发环境仍允许本地 Anvil 或 Compose 的 HTTP endpoint。相同 RPC 被 Treasury liquidity source 使用时，每个后台 refresh target 都必须先通过链身份校验，才能读取同一 block 的 `treasury()` 与 `balanceOf()`；只有全量 target 成功，新的 hot-state generation 才会发布，任何未验证链都不能进入请求期准入证据。
...
- `LocalSettlementVerifierPolicy` 在构造期 fail fast：malformed policy object, inherited policy fields and policy array fields must be rejected before field access，之后 required own `verifierVersion` 必须非空，required own `enabledChainIds` 和 `tokenWhitelist` 必须非空且不能包含重复项，chain id 必须是正安全整数，token whitelist entries、`settlementAddress` 和 `trustedSignerAddress` 必须是真实字符串且是 20-byte hex address。`trustedSignerOverlapAddresses` 必须是 own array，最多四个 non-zero 20-byte 地址，且与 primary 或彼此均不能大小写无关重复。错误 policy 不能以 inherited fields、空白版本、空白 allowlist、重复 whitelist、畸形地址或 JavaScript regex coercion 进入 `/submit` 结算验证路径。
- `LocalSettlementVerifier` snapshots `LocalSettlementVerifierPolicy` at construction after validation. External callers must not be able to mutate `verifierVersion`, enabled chains, token whitelist, settlement domain, primary signer or overlap signer array after construction and silently change settlement verification or audit output.
- `LocalSettlementVerifier.verify()` rejects malformed root payloads, missing or inherited root `quoteId` / `request` fields, `quoteId` values that are not primitive-string 1-128 character `SafeIdentifier` values, missing `request.quote`, and inherited `request` / `request.quote` required fields before signature shape, canonical checks or settlement policy evaluation. It also validates quote fields before policy checks: chain id and deadline must be own positive safe integers, user/token fields must be own 20-byte address strings, amount fields and nonce must be own canonical positive uint strings without leading zeros, and the signature must be an own 65-byte hex string. After canonical shape validation, settlement verification only converts `amountOut` and `minAmountOut` to BigInt for the minimum-output comparison, then recovers the EIP-712 signer against the configured `settlementAddress` and requires membership in the startup-snapshotted primary-plus-overlap signer set. Public `/submit` should still be protected by gateway validation first, but direct service calls must not turn malformed settlement verification input into unclassified `TypeError` failures or JavaScript regex coercion from boxed `String` identifiers.
...
- The API gateway acquires a quote-scoped reservation before `/submit` enters settlement verification. Local development uses `InMemorySubmitReservationStore`; non-local deployments use `PostgresSubmitReservationStore`, whose `INSERT ... ON CONFLICT ... WHERE expires_at <= now()` atomically gives one replica ownership and lets only expired rows be replaced using PostgreSQL server time. A concurrent owner receives `QUOTE_ALREADY_USED`/409; persistence or malformed-result failures return `SUBMIT_RESERVATION_UNAVAILABLE`/503 and fail closed. Release is conditional on the random owner token, so an expired worker cannot delete a newer lease. The default 900-second lease exceeds the maximum configured receipt wait with recovery margin. This lease reduces duplicate off-chain verification and relay work, but `RFQSettlement` nonce consumption remains the authoritative replay protection.
...
- Execution and reconciliation share `DeltaNeutralHedgePlanner`: when only `tokenOut` is the USD reference, a settlement creates `sell tokenIn/amountIn`; when `tokenIn` is a USD reference it creates `buy tokenOut/amountOut`, including stablecoin-to-stablecoin inventory rebalancing. Startup rejects managed pairs with no USD reference, and planner ambiguity fails closed after settlement rather than emitting a directionally wrong CEX order.
...
- PnL attribution rejects malformed root payloads, missing `quote` objects, and inherited root or signed quote required fields before field access. It validates `quoteId`、`settlementEventId`、`snapshotId` and the derived `pnlId` as `SafeIdentifier`, requires settlement `realizedAt` and snapshot `valuationObservedAt` to be canonical UTC timestamps, and validates the full signed quote payload before recording. `QuoteSnapshotPnlValuationProvider` loads the immutable persisted snapshot, requires its chain/token pair to match the settled quote, and obtains both token decimals from the trusted registry. `quote_snapshot_edge_v1` then computes `fairAmountOut` with exact BigInt decimal normalization, records `fairAmountOut - amountOut` in tokenOut base units, and computes bps against `fairAmountOut`. It explicitly excludes fees、gas and hedge execution. A separate `hedge_fill_net_v1` projection is completed only by exact fee reconciliation with frozen route accounting; third-asset commissions remain unavailable. Gross totals group by `(chainId, tokenOut)`, completed net totals group by `(chainId, valuationToken, valuationAsset)`, and every gross trade has one net status record. Invalid or missing evidence must fail closed instead of being counted as zero.
- Hedge intent creation failure after settlement is best-effort but risk-aware. `/submit` still returns HTTP 202 without `hedgeOrderId`, records `rfq_hedge_intent_errors_total{reason="HEDGE_INTENT_FAILED"}`, and updates Hedge Service failure pressure on the planned hedge token. Follow-up quotes validate both pair-leg pressure reads and use the maximum as quote risk penalty.
...
- 合法的 durable PostgreSQL hedge envelope 还可以携带 venue、成交额证据、费用对账状态和 commission 汇总。Execution Service 必须严格校验这些字段的格式与状态组合，但不能因其比内存实现更完整而把 queued intent 误报为 `HEDGE_INTENT_FAILED`；回归测试直接覆盖 `PostgresHedgeService` 的 queued response shape，完整 E2E 进一步要求该任务可被 production worker claim。
...
`make settlement-indexer-e2e` 进一步使用真实 Anvil、PostgreSQL store、`SettlementIndexerWorker`、`PostTradeReconciliationWorker`、`HedgeWorker` 和 `HedgeFeeWorker` 验证完整恢复语义。第一笔成交经过 `/submit` 后必须保留 queued hedge，indexer 必须返回 duplicate，随后签名 Binance fixture 返回精确 fill 与 fee evidence，系统收敛 inventory 和 net PnL；第二笔只由钱包广播，indexer 必须独立发现。测试再通过 `evm_snapshot`/`evm_revert` 制造替代链，要求孤儿事件转为 non-canonical、quote 恢复 `signed`、可逆链派生状态清除、已发生的 CEX 证据保留、inventory 重建且 cursor 沿替代链继续推进。该门禁必须显式确认并只能连接 disposable loopback database，Contract CI 在迁移后强制执行。
