...
- Gateway 关闭 Fastify 默认 request logging，改用统一 Pino JSON completion record。记录只包含 service、ISO time、level、traceId、method、route template、statusCode 和 durationMs，不写 dynamic URL、query、request body 或 headers；`/health`、`/ready`、`/metrics` 成功日志降为 debug，4xx rejection 为 warn，5xx failure 为 error。`RFQ_LOG_LEVEL` 只接受 `debug|info|warn|error`，非法值在 standalone startup fail fast。
...
- Standalone backend process delegates signal and deadline ownership to `backend/src/runtime/process-shutdown.ts`, using the shared bounded shutdown controller for `SIGTERM` and `SIGINT`. The first signal starts `RFQ_SHUTDOWN_TIMEOUT_MS` before closing Fastify; successful close clears the deadline, while deadline expiry or a second signal emits a bounded error code and exits 1 instead of waiting for kubelet `SIGKILL`.
...
- Fastify 使用统一 `RateLimiter` 接口保护 `/quote`、`/submit`、状态查询以及全局/pair quote-control 管理接口；默认窗口为 60 秒，默认限额为 quote 120 requests / 60 seconds、submit 60 requests / 60 seconds、status 300 requests / 60 seconds。Local development 可使用 `InMemoryRateLimiter`、明文 `redis://` 或显式 `rateLimit:false` 基准测试，但任何非本地 `NODE_ENV` 都强制 `RFQ_RATE_LIMIT_BACKEND=redis`、要求 `RFQ_REDIS_URL` 使用 `rediss://`，并在启动阶段拒绝 `buildServer({ rateLimit: false })`，使嵌入式调用和标准入口遵守同一生产策略；PostgreSQL 同时必须使用 `sslmode=verify-full`。`RedisRateLimiter` 使用单个 Lua script 原子读取 fixed-window 计数和 TTL，并预留不超过剩余全局容量的有界 permit batch；进程通过单键并发合并和 monotonic-TTL cache 消费本地 lease。默认 batch 为 8、local bucket 上限为 10000，分别由 `RFQ_RATE_LIMIT_LOCAL_PERMIT_BATCH_SIZE` 和 `RFQ_RATE_LIMIT_MAX_LOCAL_BUCKETS` 调整；进程崩溃或 eviction 只会浪费未用 permit，Redis 不会在窗口内重新发放，因此不会超出全局阈值。`x-ratelimit-remaining` 是保守的本地 lease 余量，不是跨副本实时精确值。Malformed config、dependency、script result、runtime input 和 unsafe client identity 都会在写状态或返回决策前被拒绝；配置在构造期 snapshot。默认 `RFQ_TRUST_PROXY=false`，限流身份使用直接 socket IP；只有可信代理清洗 `x-forwarded-for` 后才可启用代理身份。trusted forwarded identity exceeding 128 characters or outside `[A-Za-z0-9_.:-]` returns `INVALID_REQUEST`/400 before limiter state mutation。Redis error 不会 fail open，而是返回 `RATE_LIMIT_UNAVAILABLE`/503，并通过 `rateLimitStore` readiness 和 `rfq_dependency_status` 暴露；正常超限保持 `RATE_LIMITED`、HTTP 429、`Retry-After` 和 `x-ratelimit-remaining` 契约。
...
- PnL 分页参数错误：`limit` 只接受规范十进制 `1..100`，`cursor` 必须是服务端返回的 `pnl1_` opaque token；重复、数组、未知或不可规范解码的参数返回 HTTP 400、`INVALID_REQUEST`，且不得进入 PnL store。
...
Gateway 应保持薄层，不执行重计算。序列化和校验必须足够快，避免成为 quote path 瓶颈。`GET /pnl` 不允许把机构全部历史加载到进程或编码为单个 JSON：默认页 50、最大页 100，使用稳定创建时间快照和 `(realizedAt, pnlId)` keyset；全局 totals 由数据库聚合，明细查询只取 `limit + 1` 行。
