...
- 校验地址、chainId、amount 和 slippageBps，并按 OpenAPI schema 拒绝错误 JSON primitive 类型；Gateway 不应把字符串数字、布尔值或数字形式的 uint 字段隐式转换后继续处理，直接调用共享 validator 时也不能把 boxed `String` 字段隐式解包后进入 regex 校验。
- 注入 traceId。
- 统一错误响应格式。
