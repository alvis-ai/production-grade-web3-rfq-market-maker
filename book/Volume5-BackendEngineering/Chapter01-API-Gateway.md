...
- 暴露 `POST /quote`、`POST /submit`、`GET /quote/:id`、`GET /hedges/:id`、`GET /health`、`GET /ready`、`GET /metrics`。
...
- traceId 必须贯穿后端调用链。
- 当前 Fastify 实现使用 `InMemoryRateLimiter` 保护 `/quote`、`/submit`、`/quote/:id` 和 `/hedges/:id`；生产部署可替换为 Redis-backed distributed rate limit，并保持 `RATE_LIMITED` 错误契约不变。
...
测试 request validation、error mapping、rate limit、health、readiness、metrics、`/quote` 路由和 hedge intent 查询。
