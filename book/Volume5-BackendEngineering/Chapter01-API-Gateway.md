...
- `/ready` 当前检查 market data freshness、pricing probe、risk probe、signer sign/verify probe，以及 quote repository、inventory、execution/hedge store、settlement event store、PnL store 和 metrics exporter 的轻量 health probe。stale snapshot 会让 `marketData` 组件变为 `degraded`；pricing 或 risk probe 失败会让 `pricing` 或 `risk` 组件变为 `degraded`；signer 无法签名或签名无法被同一 trusted signer 验证时，`signer` 组件变为 `degraded`；存储或执行依赖探针失败时对应 `quoteRepository`、`execution`、`settlementEventStore`、`pnl` 或 `metrics` 组件变为 `degraded`，用于阻止 Kubernetes 在错误价格输入、不可签名、不可定价、不可风控或关键状态依赖不可用时继续导流。
...
- Pricing or risk readiness probe failed：`/ready` 返回 HTTP 503/degraded，并通过 `rfq_dependency_status` 指明 `pricing` 或 `risk`；`POST /quote` 仍会在实际报价时返回 `PRICING_UNAVAILABLE` 或 fail-closed `RISK_REJECTED`。
...
测试 request validation、Fastify parser error mapping、not-found handler、body limit、CORS allowed origin、CORS preflight rejection、security headers、HSTS toggle、graceful shutdown signal handling、error mapping、rate limit、health、readiness market data degraded、readiness pricing degraded、readiness risk degraded、readiness signer degraded、readiness storage dependency degraded、metrics、`/quote` 路由、settlement event 查询、hedge intent 查询、PnL summary 查询，以及成功和失败响应的 `x-trace-id` 传播。
