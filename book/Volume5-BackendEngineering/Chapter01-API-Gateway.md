...
- `backend/src/main.ts` is a thin composition root: it constructs shared dependencies, owns resource lifecycle hooks, then delegates HTTP policy to `backend/src/api/http-boundary.ts` and endpoint behavior to `backend/src/api/trading-routes.ts`. `backend/src/runtime/gateway-runtime.ts` owns `BuildServerOptions` validation and gateway dependency selection, `backend/src/runtime/market-runtime.ts` owns market data、pricing、token-registry and risk-policy runtime assembly, and the remaining environment/process primitives live under `backend/src/runtime/`. `make api-composition-check` caps the entrypoint at 500 lines and rejects direct route registration in `main.ts`, preventing unrelated API、market-data and lifecycle changes from converging on one monolithic file.
...
- `quoteControl` 是独立 readiness 组件：每次 probe 读取并验证共享状态，同时刷新 `rfq_quote_paused`；状态可读时保持 `ok`，即使当前处于有意 pause，存储或状态格式异常时变为 `degraded`。这一区分避免把运维暂停误报为基础设施故障，同时保证重启副本的 gauge 快速收敛，且无法证明 enabled 时所有新 quote fail closed。
...
- Quote control paused：`POST /quote` 在定价和签名前返回 `QUOTE_PAUSED`/503；既有 signed quote 的 `/submit` 和状态查询不被该开关阻断。
