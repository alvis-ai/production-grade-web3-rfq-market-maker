...
Gateway 需要输入校验、限流、CORS allowlist 和日志脱敏。不能把签名服务或内部管理接口暴露到公网。
...
测试 request validation、Fastify parser error mapping、body limit、CORS allowed origin、CORS preflight rejection、error mapping、rate limit、health、readiness market data degraded、readiness signer degraded、metrics、`/quote` 路由、settlement event 查询、hedge intent 查询、PnL summary 查询，以及成功和失败响应的 `x-trace-id` 传播。
