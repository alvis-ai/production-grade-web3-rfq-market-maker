...
- Binance order、`myTrades`、`/time` 与 `exchangeInfo` 响应统一在 JSON 解码前执行 2 MiB 流式上限。带超限 `Content-Length` 的响应会在读取 body 前拒绝，chunked 或解压后的实际字节超过上限也会中止；订单路径映射为可重试的 `BINANCE_RESPONSE_INVALID`，校时与规则路径分别保留 `BINANCE_TIME_SYNC_FAILED` 和 `BINANCE_SYMBOL_RULES_INVALID`。Signed response 只读取一次，不能为了识别 `-1021` 克隆并重复无界解码。
