...
- `PostgresHedgeService` 的 queued response 与内存实现共享 required fields，但还可能包含 venue、execution evidence、fee reconciliation 和 commission fields。`execution-service-hedge-result-validation.ts` 对完整 union 做 closed-envelope 校验；Contract CI 的 core-flow E2E 从 `/submit` 开始证明合法 PostgreSQL row 不会被错误降级为 `HEDGE_INTENT_FAILED`，并能继续被 production hedge/fee workers 执行。
- Hedge failure penalty config is validated at construction: `failurePenaltyBps` and `maxFailurePenaltyBps` must be own positive safe integers, each must be at most 10000 bps, `failurePenaltyBps` must not exceed `maxFailurePenaltyBps`, and `failureLookbackMs` must be a positive safe integer no greater than one day. API runtime reads the corresponding `RFQ_HEDGE_FAILURE_PENALTY_BPS`、`RFQ_HEDGE_MAX_FAILURE_PENALTY_BPS` and `RFQ_HEDGE_FAILURE_LOOKBACK_MS` values with canonical base-10 parsing. Invalid config fails fast before `/submit` can accept a settlement whose follow-up quote risk feedback would be nonsensical.
- `HedgeService` snapshots `HedgeServiceConfig` at construction after validation. External callers must not be able to mutate failure penalty increments, caps or lookback after construction and silently change follow-up quote risk feedback.
...
- Non-local runtime uses `PostgresHedgeService`: one unique `hedge_orders` row per settlement event, deterministic cross-replica hedge ids, DB timestamps, durable queued/filled/failed transitions, and idempotent conflict validation. Migration 029 adds `risk_failure_at` plus a partial `(chain_id, lower(token_address), risk_failure_at)` index. A database trigger writes this timestamp only on the first failed transition and preserves it across later updates; this also keeps an old worker compatible while the migration and new runtime roll out. Quote risk counts only recent failed rows joined to `settlement_events.canonical=TRUE`; reorg-orphaned or expired failures no longer widen quotes, while unresolved recent failures remain shared across API replicas.
...
- API and worker use the same bounded `RFQ_BINANCE_SYMBOL_RULES_MAX_AGE_MS` cache over unsigned `GET /api/v3/exchangeInfo?symbol=...`. Route health requires `TRADING`, spot `LIMIT` support, exact base/quote assets, and exact equality between configured raw-unit step/price tick and live `LOT_SIZE`/`PRICE_FILTER`; each candidate order also satisfies live min/max quantity, price tick/range and `MIN_NOTIONAL`/`NOTIONAL` bounds using `BigInt` decimal arithmetic. API readiness prevents stale executable depth from remaining quoteable, worker readiness blocks drifted routes, and the worker performs this preflight before `prepareRoute` and `authorizeSubmission` so a failed metadata request never creates false submission ambiguity.
...
- Binance order 与 `myTrades` 的 signed response 在同一个 request deadline 内完成 headers、2 MiB 流式读取和 JSON 解码；收到 headers 后停滞仍会被 AbortController 中止并映射为 `BINANCE_REQUEST_FAILED`，保持 query-by-client-id 的歧义恢复语义。成功的 `/time` 与 `exchangeInfo` body 使用相同 deadline 和字节上限；不需要解析的非 2xx body 立即取消。带超限 `Content-Length` 的响应会在读取前拒绝，chunked 或解压后的实际字节超过上限也会中止；格式与大小错误分别保留订单、校时和规则路径的 fail-closed 错误契约。Signed response 只读取一次，不能为了识别 `-1021` 克隆并重复无界解码。
...
- 新 execution route 上线前，使用独立 Binance Spot Testnet key 执行 `make binance-testnet-integration-check`。canary 固定连接 `https://testnet.binance.vision`，要求显式 `place-and-cancel` 确认，并在真实盘口之外至少保留默认 100 bps 距离；生产 adapter 必须完成 absent、pending、query、cancel、terminal 和空 `myTrades` 证据，任一累计成交量非零都判定失败。
- Backend CI 的 `make hedge-worker-e2e` 使用 disposable PostgreSQL 与验证 HMAC、symbol rules、订单查询、成交响应和 `myTrades` 的确定性 Binance fixture。验证从 canonical settlement trigger 开始，必须经过 production reconciliation worker、`HedgeWorker`、`HedgeFeeWorker` 和各 PostgreSQL store，最终证明 `LIMIT GTC` policy、base-and-quote evidence、inventory delta、commission 与 `hedge_fill_net_v1` 同时收敛；直接调用 store terminal methods 的测试不能替代该门禁。
- Contract CI 的 `make settlement-indexer-e2e` 将上述 venue 证据链与真实 Anvil `/submit`、独立 indexer、wallet-only recovery 和 replacement-chain reorg 放入同一个测试。这个门禁专门捕捉 API、PostgreSQL response shape、worker claim、链上事件和 reorg policy 之间的组合漂移，不能由彼此独立的 contract/indexer/worker 测试替代。
...
- Malformed hedge config, clock dependency, intent and risk feedback root payloads are rejected before field access or state mutation, so post-trade hedge failures cannot turn into unclassified `TypeError` paths or partial failure-pressure updates.
- Persistent hedge rows keep `externalOrderId` nullable while an intent is only queued internally, but any non-null external order reference must be non-empty so venue reconciliation can trace it. `updatedAt` records the latest row transition while `risk_failure_at` preserves the first terminal failure time and `createdAt` remains the original intent time.
...
测试 hedge skipped、route selected、step-size rounding、live `exchangeInfo` route drift、exact price/quantity/notional filters、submission-authorization-before-preflight regression、HMAC signing、query-before-submit、`-1021` 单飞校时与重签、venue reject、partial fill/pending retry、数据库时钟驱动的最大挂单时长、取消前意图持久化、取消与成交竞态、ambiguous cancel recovery、四窗口 lease 边界、lease conflict、idempotent retry、`myTrades` pagination、trade ID safety、eventual-consistency retry、multi-asset commission、immutable fill conflict、hedge intent creation failed does not rollback settlement、follow-up quote risk penalty、failure penalty config fail-fast、hedge status store unavailable 和 worker metrics emission。CI 中的 `make binance-testnet-check` 使用签名 REST fixture 验证同一生命周期和可成交价格拒绝；目标环境仍须使用 testnet 资金执行 credentialed canary，fixture 不能证明真实 venue 权限、时钟或网络路径。
...
- [Binance Spot API: Exchange information](https://github.com/binance/binance-spot-api-docs/blob/master/rest-api.md#exchange-information)
- [Binance Spot API: Filters](https://github.com/binance/binance-spot-api-docs/blob/master/filters.md)
- [Binance Spot API: Cancel order](https://github.com/binance/binance-spot-api-docs/blob/master/rest-api.md#cancel-order-trade)
- [Binance Spot Test Network](https://developers.binance.com/en/docs/products/spot/testnet/rest-api)
