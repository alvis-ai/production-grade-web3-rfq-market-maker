...
- Hedge intent and risk feedback inputs are validated before writing hedge state: `settlementEventId` and `quoteId` must be `SafeIdentifier` values with 1-128 characters matching `[A-Za-z0-9_:-]`, `chainId` must be a positive safe integer, `token` must be a 20-byte address, `amount` must be a positive uint string, and `side` / `reason` must match the supported enum values.
