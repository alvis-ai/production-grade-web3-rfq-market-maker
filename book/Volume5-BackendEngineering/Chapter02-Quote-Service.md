...
sequenceDiagram
  ...
  Q->>P: price
  Q->>R: evaluate
  Q->>E: reserve exposure when approved
  Q->>I: admit(snapshot + request + route + risk)
  I->>J: atomic authorized state + Stream event
  alt approved
    Q->>S: signQuote
...
- 生产 Redis 路径在 base risk 通过后先完成 exposure reserve，再以一次 `admit` Lua 原子写入 snapshot、request、route、risk、idempotency quote binding 和 cumulative `authorized` Stream event。若 admission 失败而 exposure 已成功，服务先 best-effort release，再返回 `QUOTE_STORE_UNAVAILABLE`，不得调用 Signer。PostgreSQL exposure projector 的 prepared-or-newer barrier 接受 `authorized` projection marker，因此异步审计顺序不进入报价响应延迟，也不会产生可恢复的外键告警风暴。PostgreSQL、自定义和回滚 store 仍保留独立 `prepare`/`authorize` 能力。
...
ADR-0030 到 ADR-0032 将 signer audit/finalization 合并为 signer 侧原子提交、使用 SHA-cached Lua，并把 production Redis 的 preparation/authorization 合并为 exposure 之后的一次 admission。真实 Redis/PostgreSQL 集成要求 8 个并发同内容 admission 只写 1 个 `authorized` event，冲突 policy fail closed，随后 signer 原子提交 1 个 `finalized` event 和 1 个 audit event，PostgreSQL 仅凭两个 cumulative issuance events 恢复完整状态。重建 Compose 后的独立 concurrency 1 窗口为零错误、p50 11.95 ms、p99 16.51 ms、80.83 requests/s；上一 SHA-cached 基线为 p50 13.00 ms、p99 18.55 ms、75.15 requests/s。`authorization_persistence`、exposure、idempotency、signing 平均分别为 1.44、1.44、1.12、5.07 ms。改善可测，但 p50 < 10 ms 仍未通过，下一阶段应评估 exposure/issuance 同槽协议或减少 signer transport/commit 成本，且必须保留跨副本 exact exposure 与签名前 durable authorization。
