...
- approved risk decision 进入 Signer 前，生产 runtime 必须读取同一 block 的 `RFQSettlement.treasury()` 和 `tokenOut.balanceOf(treasury)`，再将证据与 `amountOut` 交给 Redis/Valkey quote exposure ledger。ledger 以 chain-scoped lease 串行化多副本 VaR/Delta 评估，并用 Lua 原子检查 `(chainId, tokenOut)` Treasury 聚合；余额不足返回内部 `TREASURY_LIQUIDITY_INSUFFICIENT`，RPC、hot state、Redis durability 或 PostgreSQL mirror health 异常返回 `RISK_ENGINE_UNAVAILABLE`，都不能调用 Signer。该改造移除了 exposure/VaR 阶段的 PostgreSQL 查询，但 snapshot/quote/risk 审计和其他动态 guard 仍有同步持久化边界。
