...
- approved risk decision 进入 Signer 前，生产 runtime 必须从已经预热的 Treasury hot view 读取目标 token 的新鲜 balance/block evidence，再将证据与 `amountOut` 交给 Redis/Valkey quote exposure ledger。后台 refresher 对全部 managed chain/token target 完成校验后才原子发布 generation；请求不回源 RPC，缺失、过期或不完整状态一律 fail closed。ledger 以 chain-scoped lease 串行化多副本 VaR/Delta 评估：fused acquire/read Lua 返回一致 token delta，fused commit/unlock Lua 原子检查 `(chainId, tokenOut)` Treasury 聚合、写入 exposure/Stream 并释放 owner。余额不足返回内部 `TREASURY_LIQUIDITY_INSUFFICIENT`，hot state 或 Redis durability 异常返回 `RISK_ENGINE_UNAVAILABLE`，都不能调用 Signer。该改造移除了 exposure/VaR 阶段的 PostgreSQL 查询和 Treasury request-time RPC；issuance mirror 与 exposure mirror 使用 bounded backlog、error 和 stalled-progress 告警。动态 toxic-flow 与 daily-loss provider 仍会在 risk evaluation 中查询 PostgreSQL，是尚待迁入 versioned hot state 的剩余同步数据库边界。
...
ADR-0021 已以 replicated Redis/Valkey issuance journal 承担同步 idempotency、nonce、authorization 和 hot quote state，再异步投影 PostgreSQL。最终真实依赖诊断在 concurrency 1 得到 p50 19.30 ms / p99 42.19 ms、零错误；修复两个审计流的 projection barrier 后，concurrency 5 得到 p50 35.91 ms / p99 54.94 ms，但出现 8 个 base-risk fail-closed，因此不是验收通过。两条 Stream 的 backlog 均回到 0 且 mirror error 为 0。当前实现仍不能称为满足高频报价 SLO；下一阶段必须把 dynamic toxic-flow 与 daily-loss 的 request-time PostgreSQL evidence 迁到 versioned hot state，并继续减少 signer 与 issuance Redis round trips。
