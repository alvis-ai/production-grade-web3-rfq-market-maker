...
ADR-0023 进一步以 generation CAS 消除 exposure reserve 的分布式 lease 临界区；ADR-0025 则用有界原生 HTTP(S) keep-alive pool 替换 API 到 signer 的全局 Fetch adapter。最终镜像的 concurrency 1 结果为 p50 12.72 ms、p99 17.56 ms、signing 平均 5.48 ms、零错误，相比上一镜像 p50 14.88 ms、p99 19.44 ms、signing 6.41 ms 有可重复改善。两个 clean concurrency 5 窗口为 p50 24.39-26.27 ms、p99 36.84-40.47 ms、零错误；另一个窗口曾因 `issuance_persistence` 单点进入 250 ms bucket 而得到 p99 135.07 ms，虽然后续无 backlog/mirror error 且未复现，也必须保留为尾延迟风险证据。当前仍不能声明 p50 或生产稳定 p99 达标，后续重点转为 exposure CAS 与 serial Redis idempotency/authorize/finalize 边界。
