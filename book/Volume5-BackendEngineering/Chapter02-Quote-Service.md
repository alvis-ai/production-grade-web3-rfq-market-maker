...
- approved risk decision 进入 Signer 前，生产 runtime 必须读取同一 block 的 `RFQSettlement.treasury()` 和 `tokenOut.balanceOf(treasury)`，再将证据与 `amountOut` 交给 quote exposure store。store 以 `(chainId, tokenOut)` 串行化多副本预留；余额不足返回内部 `TREASURY_LIQUIDITY_INSUFFICIENT`，RPC 或证据异常返回 `RISK_ENGINE_UNAVAILABLE`，两者都不能调用 Signer。
