...
- PostgreSQL stores `quotes.deadline` as positive BIGINT Unix seconds, matching `SignedQuote.deadline`, EIP-712 `uint256 deadline`, signer verification and settlement verification. It must not be stored as `timestamptz`, because timezone conversion would obscure the exact signed integer.
