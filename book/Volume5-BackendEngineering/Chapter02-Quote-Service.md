...
- Market snapshot persistence and idempotency quote binding are independent writes after the idempotency owner has created the quote id, so `QuoteService` starts them together and waits for both before routing. Inventory skew and the two directional hedge-risk penalties are likewise evaluated concurrently after route persistence. Any branch failure preserves the same fail-closed error mapping; concurrency changes latency, not the signing order or audit requirements.
...
- Internal inventory `routeId` uses the full normalized `tokenIn` and `tokenOut` addresses plus `chainId`; address prefixes are not sufficient because two assets can share the same first bytes. After route validation, `QuoteService` must call `QuoteRepository.saveRouteDecision()` before inventory adjustment or pricing. A write failure maps to `QUOTE_STORE_UNAVAILABLE`, best-effort marks the requested quote failed, and blocks pricing, risk and signer access.
...
- `QuoteService` persists RiskDecisionStore audit records before signer access. Approved 和 rejected decision 都必须写入 `RiskDecisionStore`，返回记录必须与输入的 quote、decision、reason 和 policy 完全一致；approved 记录的 `riskDecisionId`、`policyVersion` 与网关 `traceId` 随后作为不可拆分的授权上下文传入远程 Signer。审计写入失败或返回畸形/冲突证据按 `QUOTE_STORE_UNAVAILABLE` 处理，best-effort 将 requested quote 标记为 `failed`，并阻断 Signer。
- approved risk decision 进入 Signer 前，生产 runtime 必须从已经预热的 Treasury hot view 读取目标 token 的新鲜 balance/block evidence，再将证据与 `amountOut` 交给 Redis/Valkey quote exposure ledger。后台 refresher 对全部 managed chain/token target 完成校验后才原子发布 generation；请求不回源 RPC，缺失、过期或不完整状态一律 fail closed。ledger 以 chain-scoped lease 串行化多副本 VaR/Delta 评估，并用 Lua 原子检查 `(chainId, tokenOut)` Treasury 聚合；余额不足返回内部 `TREASURY_LIQUIDITY_INSUFFICIENT`，hot state、Redis durability 或 PostgreSQL mirror health 异常返回 `RISK_ENGINE_UNAVAILABLE`，都不能调用 Signer。该改造移除了 exposure/VaR 阶段的 PostgreSQL 查询和 Treasury request-time RPC，但 snapshot/quote/risk 审计和其他动态 guard 仍有同步持久化边界。
...
- PostgreSQL 幂等记录必须先抢占 `(principal_id, idempotency_key)`，再在创建 nonce 后、写入 requested quote 前绑定 `quote_id`。进程若在签名链路中崩溃，未绑定且过期的租约可以换 owner；已绑定租约不能重新签名。租约过期后若 `quotes` 已有完整 signed payload，则恢复并缓存原响应；否则把仍为 requested 的 quote 和幂等请求一起失败终结。
- PostgreSQL 幂等抢占的无冲突常见路径使用单条 `INSERT`，只在唯一键冲突后进入事务并锁定已有记录。requested-to-signed 的常见路径使用 identity-bound compare-and-set upsert；只有 CAS miss 才回读当前 quote 进行 immutable replay/invariant 判定，nonce 唯一约束冲突由数据库 constraint name 映射为稳定错误。该优化减少网络往返，但不放宽 quote identity、状态机或 nonce 唯一性。
