...
Quote Service 依赖 Market Data、MarketSnapshotStore、Pricing、Risk、Signer、Quote Repository、RiskDecisionStore 和 Metrics。无 `DATABASE_URL` 的本地开发使用 in-memory repositories；任何非本地 runtime 强制 PostgreSQL。默认完整 PostgreSQL composition 使用 `PostgresQuoteIssuanceStore` 将 snapshot/request/route/idempotency、risk authorization、signed response 分别融合为 `prepare`、`authorize`、`finalize` 三条原子语句；只要注入任一自定义 repository，就保留原有逐仓储路径，避免混用两套 ownership。当前实现会在 pricing 和 signing 之前校验 market snapshot 的 `observedAt`，该字段必须是 `Date.prototype.toISOString()` 生成的 canonical UTC ISO timestamp；超过 freshness window 的 stale market data、明显来自未来的 snapshot、date-only/natural-language timestamp 或会被 JavaScript 自动归一化的非法日期都会返回 `MARKET_DATA_UNAVAILABLE`，避免签出过期价格或接受错误时钟的数据源。
...
sequenceDiagram
  ...
  I->>DB: atomic authorization
  alt approved
    Q->>S: signQuote
    Q->>I: finalize(signed quote + response)
    I->>DB: atomic finalization
    Q-->>API: QuoteResponse
  else rejected
    Q-->>API: Risk error
  end
...
- 默认 PostgreSQL 路径在 base risk 计算后调用 `prepare`，以一条 data-modifying CTE 原子写入 market snapshot、requested quote、route attribution 和 idempotency quote binding。它必须在 exposure reserve 之前 commit，保证 Redis Stream mirror 写 `quote_exposure_reservations` 时外键父记录已存在。prepare 失败返回 `QUOTE_STORE_UNAVAILABLE`，不得预留 exposure 或调用 Signer。
...
- `QuoteService` persists risk authorization before signer access. 默认 fused 路径以 `authorize` 原子更新 prepared quote 并插入 risk decision；自定义路径写 `RiskDecisionStore`。Approved 和 rejected decision 返回记录都必须与输入的 quote、decision、reason 和 policy 完全一致；approved 记录的 `riskDecisionId`、`policyVersion` 与网关 `traceId` 随后作为不可拆分的授权上下文传入远程 Signer。审计失败或畸形/冲突证据按 `QUOTE_STORE_UNAVAILABLE` 处理、释放已完成的 exposure reservation，并阻断 Signer。
- 签名成功后，默认 fused 路径以 `finalize` 原子写 signed payload 并完成 idempotency response；自定义路径继续调用 `saveSigned` 和独立 idempotency completion。三阶段把常见 idempotent PostgreSQL 路径从八次操作降为四次查询（含最初 acquire），但 PostgreSQL 仍在同步响应路径上。
...
ADR-0020 的真实依赖诊断在 concurrency 1 得到 p50 21.83 ms / p99 33.39 ms，在 concurrency 5 得到 p50 36.66 ms / p99 49.72 ms，全部请求成功。并发尾延迟改善且 p99 单次达线，但 p50 仍明显失败；当前实现不能称为满足高频报价 SLO。要进入 p50 10 ms 以内，下一阶段必须以 replicated Redis/Valkey issuance journal 承担同步 durable admission 和 hot quote state，再异步投影 PostgreSQL，而不是继续增加同步 SQL 优化。
