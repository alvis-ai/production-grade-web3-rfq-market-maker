...
sequenceDiagram
  ...
  Q->>P: price
  Q->>R: evaluate
  Q->>I: joint admit(exposure + snapshot + request + route + risk)
  I->>E: same-slot exact exposure CAS
  I->>J: same Lua authorized state + Stream event
  alt approved
    Q->>S: signQuote
...
- 生产 Redis 路径以同一个 `{quote-state}` Lua 原子完成 exact exposure reserve、snapshot/request/route/risk、idempotency quote binding 和 cumulative `authorized` Stream event。双重 preflight 全部通过后才依次 apply exposure 与 issuance；exposure reject 不写 approved issuance，issuance conflict 不写 exposure。联合 store 仅在 Redis authority、hash tag、AOF 与 replica ACK policy 完全一致时装配。PostgreSQL exposure projector 的 prepared-or-newer barrier 接受 `authorized` projection marker，因此异步审计顺序不进入报价响应延迟。PostgreSQL、自定义和回滚 store 仍保留独立能力路径。
...
- `QuoteService` persists risk authorization before signer access. 生产 Redis 路径以 `admit` Lua 一次创建 cumulative `authorized` state 并追加包含完整 preparation 与 approved/rejected evidence 的事件；逻辑授权哈希不包含生成时间，因此并发同内容重放只产生一个事件，而 policy 或 signer commitment 变化会 fail closed。PostgreSQL 兼容路径仍使用 fused SQL，自定义路径写 `RiskDecisionStore`。Approved 和 rejected decision 返回记录都必须与输入的 quote、decision、reason 和 policy 完全一致；approved 记录的 `riskDecisionId`、`policyVersion` 与网关 `traceId` 随后作为不可拆分的授权上下文传入远程 Signer。审计失败或畸形/冲突证据按 `QUOTE_STORE_UNAVAILABLE` 处理、释放已完成的 exposure reservation，并阻断 Signer。
- `durable_authorization_wait_v1` 只在联合准入与 signer atomic finalization 同时启用时允许低延迟并行。Signer 可以在 Lua 发送边界开始解析请求和计算 EIP-712 digest，但 Local/KMS key access 必须等待 Redis 中 exact `authorized` record；`RFQ_SIGNER_AUTHORIZATION_WAIT_MS` 默认部署值为 10ms，只对 missing key 做 1ms 有界重试，任何 malformed/rejected/conflicting record 立即 fail closed。准入响应歧义时，Quote Service 先恢复 signer atomic finalization：已提交则返回核验后的 response，确认未提交才 release exposure，未知状态保留 deadline-bound reservation。
...
- approved risk decision 进入 Signer 前，生产 runtime 必须从已经预热的 Treasury hot view 读取目标 token 的新鲜 balance/block evidence，再将证据与 `amountOut` 交给 Redis/Valkey quote exposure ledger。后台 refresher 对全部 managed chain/token target 完成校验后才原子发布 generation；请求不回源 RPC，缺失、过期或不完整状态一律 fail closed。reserve 先由 Lua 清理过期项并返回一致 token delta 与 chain generation，在内存计算 VaR/Delta 后以 CAS Lua 复核 generation、`(chainId, tokenOut)` Treasury 聚合及 exact limits；冲突必须基于新 generation 重算且受 deadline 约束，不能提交旧风险快照。余额不足返回内部 `TREASURY_LIQUIDITY_INSUFFICIENT`，hot state、重试耗尽或 Redis durability 异常返回 `RISK_ENGINE_UNAVAILABLE`，都不能调用 Signer。动态 toxic-flow、daily-loss、USD-reference、hedge penalty、quote-control 和 settlement-indexer evidence 同样在启动时预热并按 `RFQ_HOT_STATE_REFRESH_INTERVAL_MS` 后台更新；报价只读不可变 process-local generation。PostgreSQL/RPC 因此退出默认报价计算路径，保留为控制面、审计源和后台刷新依赖；Redis/Valkey 仍承担跨副本 exposure、issuance 与 rate-limit 一致性。
...
ADR-0026 到 ADR-0029 继续缩短热路径：process-local exposure generation 让 warm reserve 直接提交 CAS；同链本地队列避免一个进程并发提交已知过期 generation；Redis issuance 只返回可独立核验的 preparation/finalization hash 或完整 risk record；gateway 与 local Compose signer 使用 pinned WASM secp256k1；Redis rate limiter 原子预留保守 permit batch，并在有界本地 lease 中消费。成功 `POST /quote` completion log 降为 debug，但每个请求仍进入 counter、total latency 和新增的 `rate_limit|quote_control` stage histogram。最终镜像经内容核验后，clean concurrency 1 为零错误，p50 13.37 ms、p99 31.71 ms、throughput 69.57 requests/s；rate limit 0.16 ms、quote control 0.04 ms、idempotency 1.19 ms、exposure 2.14 ms、authorization 1.56 ms、signing 3.60 ms、finalization 2.06 ms。此前两轮 clean window 为 p50 14.09-14.35 ms、p99 19.46-24.84 ms，说明本地尾延迟存在正常波动，不能把单次低值视为上界。concurrency 5 下，本地 generation 队列把 exposure 平均值从 8.65 ms 降到 4.94 ms、p99 从 51.83 ms 降到 46.67 ms，throughput 从 197.08 提升到 204.22 requests/s，零错误；p50 仍为 22.39 ms。当前明确不能声明 p50 < 10 ms，也不能用 local signer 代替 AWS KMS/TLS/cross-zone 证据；下一阶段必须融合或移出 serial durable boundaries，而不是继续优化已经低于 0.2 ms 的 routing、pricing、rate-limit 和 quote-control 计算。

ADR-0030 到 ADR-0032 将 signer audit/finalization 合并为 signer 侧原子提交、使用 SHA-cached Lua，并把 production Redis 的 preparation/authorization 合并为 exposure 之后的一次 admission。真实 Redis/PostgreSQL 集成要求 8 个并发同内容 admission 只写 1 个 `authorized` event，冲突 policy fail closed，随后 signer 原子提交 1 个 `finalized` event 和 1 个 audit event，PostgreSQL 仅凭两个 cumulative issuance events 恢复完整状态。重建 Compose 后的独立 concurrency 1 窗口为零错误、p50 11.95 ms、p99 16.51 ms、80.83 requests/s；上一 SHA-cached 基线为 p50 13.00 ms、p99 18.55 ms、75.15 requests/s。`authorization_persistence`、exposure、idempotency、signing 平均分别为 1.44、1.44、1.12、5.07 ms。改善可测，但该基线仍未通过 p50 < 10 ms。

ADR-0033 把 exposure 与 cumulative issuance admission 迁入同一 `{quote-state}` Redis Cluster slot，并在显式 capability gate 下让 signer validation/digest 与最后一条 Redis command 重叠。Signer 仍必须读取 exact durable authorization 后才能访问 key；server 复用已经验证和审计的 EIP-712 digest，gateway 仍独立恢复 signer address 并核对 finalization hash。真实 Redis 检查要求 8 个并发重放各 ledger 仅 1 个 event、exposure reject 无部分写、issuance conflict 无 exposure。最终镜像重建 Compose 后以 concurrency 1、10 次预热和 100 次 measured HTTP quote 得到零错误、p50 8.87ms、p99 16.39ms、105.16 requests/s，并通过严格 9.99ms p50 / 50ms p99 门禁。该结果证明本地独立 signer/Redis/PostgreSQL 栈满足目标，不替代 AWS KMS、TLS 与 cross-zone 生产基准。
