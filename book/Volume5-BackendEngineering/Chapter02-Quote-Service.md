...
- The PostgreSQL schema mirrors these invariants with quote status payload consistency checks: signed lifecycle states must keep the signed quote payload, rejected/failed states must keep `reject_code`, non-settlement states cannot expose settlement pointers, and submitted/settled states must keep `tx_hash` plus `settlement_event_id` while `hedge_order_id` and `pnl_id` remain optional for best-effort post-trade recovery.
