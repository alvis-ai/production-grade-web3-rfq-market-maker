...
sequenceDiagram
  ...
  S->>K: sign digest
  K-->>S: signature
  S->>R: atomic append + replica ACK
  R-->>S: durable admission
  S-->>Q: signature
  S->>R: consume admitted event
...
- 生产默认使用 `RFQ_SIGNER_AUDIT_BACKEND=redis-stream`。一次 Lua 原子操作执行 backlog 上限检查、schema-versioned event append 与 digest-based dedupe；`rediss://`、healthy AOF 和至少一个 `WAIT` replica acknowledgement 都是返回 signature 的前置条件。普通内存队列不能作为该边界。
...
KMS latency 进入 quote p99，需要监控 signer latency、signer error rate 和 queue depth。当前 reference implementation 暴露 signer latency histogram，以及 audit stream append、duplicate、backlog、replica ACK failure、mirror insert/replay 和 mirror error 指标。Redis admission 降低同步 PostgreSQL pool contention，但 AOF 与 replica ACK 仍属于计时链路，必须通过真实依赖栈 benchmark 证明效果。
