...
`PricingResult` includes amount fields and complete component bps fields. It matches backend `pricing.engine.ts` and keeps volatility plus hedge cost separate from inventory state.

当前 `formula-v3` 返回 `amountOut`、`minAmountOut`、`spreadBps`、`sizeImpactBps`、`inventorySkewBps`、`volatilityPremiumBps`、`hedgeCostBps` 和 `pricingVersion`。`spreadBps` 仍表示聚合后的总报价调整，但每个可变业务分量独立保存；`hedgeCostBps` 当前表示 Hedge Service 基于历史失败压力给出的风险成本，而不是已经发生的成交费用。`amountOut` 和 `minAmountOut` 始终是 tokenOut base units。
...
- pricingVersion、spreadBps、sizeImpactBps、inventorySkewBps、volatilityPremiumBps 和 hedgeCostBps 写入 signed quote record，用于报价回放、风险审计和用户争议解释。
...
- `formula-v3` 把 human tokenOut-per-tokenIn price 转为 tokenOut base units，最多接受 18 位价格小数且不静默截断。size impact 使用 rational USD notional；两侧均非 USD reference 的 pair 在具备独立 USD valuation feed 前不报价。
