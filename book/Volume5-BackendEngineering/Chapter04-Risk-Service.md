...
`TokenLimitRiskPolicy` 包含 `policyVersion`、`enabledChainIds`、`tokenLimits`、`restrictedUsers`、`toxicFlowScores`、`maxUserOpenNotionalUsd`、`maxPairOpenNotionalUsd`、`minLiquidityUsd` 和四个 bps 上限。每个 `TokenRiskLimit` 固定 `chainId`、`tokenAddress`、`maxAmountIn`、`minAmountOut`、`maxNotionalUsd`、`maxAbsoluteInventory`。当前稳定 reasonCode 还包含累计门禁代码 `USER_OPEN_NOTIONAL_LIMIT_EXCEEDED` 和 `PAIR_OPEN_NOTIONAL_LIMIT_EXCEEDED`；它们与其他拒绝原因一样写入 `risk_decisions`，但不向公共 API 暴露具体阈值。持久化记录要求 `policyVersion` 非空，approved decision 的 `reasonCode` 为 NULL，rejected decision 的 `reasonCode` 为非空稳定代码。PostgreSQL `risk_decisions.reason_code` CHECK constraint 必须匹配后端 `RiskRejectReasonCode` union。`quote_exposure_reservations` 单独保存 quote、chain、normalized user/pair scope、`notional_usd_e18` 和 `expires_at`，只表达签名尚可执行期间的保守潜在敞口，不替代成交后的 inventory ledger。
...
测试每个 reasonCode、cross-chain address isolation、6/18 decimals 的独立 raw-unit 限额、policy missing、policy/registry mismatch、policy config fail-fast、inventory stale、quoted spread guard、approved path、audit persistence failure、risk decision store idempotency/conflict/defensive copy、活动签名报价 exact boundary、反向 pair canonicalization、TTL expiry、failure release、PostgreSQL advisory-lock ordering 和 risk engine unavailable fail-closed 路径。风险引擎不可用测试必须同时断言 `rfq_quote_rejections_total{reason="RISK_ENGINE_UNAVAILABLE"}` 增加，且 `rfq_signer_requests_total{operation="sign"}` 仍为 0。
