...
简单系统只做全局 amount limit，但 raw units 无法跨 token 比较，同一地址也不能跨 chain 隐式共享权限。当前默认实现使用 `TokenLimitRiskEngine`：`RFQ_RISK_POLICY_JSON` 为每个 chain/token 分别给出 `maxAmountIn`、`minAmountOut` 和 `maxAbsoluteInventory` canonical uint256 string；Quote Service 传入成交后的 tokenIn/tokenOut 库存投影，Risk Engine 按各自 token 的 raw units 检查。策略仍支持 restricted user 和 per-user toxic score gate。生产系统后续在同一接口下扩展 USD notional、VaR、动态 toxic score cache 和外部 policy store。
