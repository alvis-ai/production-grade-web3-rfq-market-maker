...
测试每个 reasonCode、policy missing、inventory stale、quoted spread guard、approved path、audit failure、rejected quote persistence failure 和 risk engine unavailable fail-closed 路径。风险引擎不可用测试必须同时断言 `rfq_quote_rejections_total{reason="RISK_ENGINE_UNAVAILABLE"}` 增加，且 `rfq_signer_requests_total{operation="sign"}` 仍为 0。
