...
- Treasury 流动性聚合按 `(chainId, tokenOut)` 计数，并与 user/pair 和 token delta 同一个 Lua commit。provider health、RPC 读取、Redis AOF/replica acknowledgement、backlog、lease、hot-state freshness 或 mirror health 失败都会让 `risk` readiness degraded 并阻断新增签名。生产绝不自动回退到 PostgreSQL authority。
