...
- 基础行情预取和审计快照采样都采用进程内 single-flight cycle。若一次 RPC 或 PostgreSQL 写入超过 timer interval，后续 tick 复用在途 Promise 而不启动重叠 I/O；关闭 API 时先清除 timer，再等待在途刷新与采样完成，之后才允许连接池关闭。该边界避免行情源或数据库变慢时形成请求放大，也防止优雅关闭期间出现写入撞上已关闭连接池的竞态。每个 base-provider pair attempt 只记录 `success|failure`，每个审计采样结果只记录 `saved|unchanged|unavailable|failed`；observer 异常不改变缓存、持久化或重试结果，动态 pair、snapshotId、RPC 与数据库错误不进入 Prometheus labels。
