...
- 可选的 `RFQ_CEX_PAIRS` 使用 `chainId:tokenIn:tokenOut:exchange:symbol` 声明 Level-2 数据源；同一链上交易对可分别配置 Binance 的 `ETHUSDT` 与 Coinbase 的 `ETH-USDT`，避免错误地把一种交易所 symbol 复用于另一种协议。监控器只消费已完成快照与增量衔接的订单簿，多个有效来源的 mid price 取中位数、可用近端深度求和；某个来源断线或发生 sequence gap 时立即清空该来源并退化到剩余来源，不把旧订单簿继续作为可报价行情。
- Binance adapter 先建立 WebSocket 并缓冲增量，再读取 REST depth snapshot；它丢弃不晚于 `lastUpdateId` 的事件，并要求首个及后续事件覆盖前一个 update id 的下一个序号。缓冲溢出、REST 超时、非法 payload 或 sequence gap 都会触发重连与重新同步。Coinbase adapter 同样要求收到目标 product 的 full snapshot 后才进入 ready，并在断线时清空本地 book。
