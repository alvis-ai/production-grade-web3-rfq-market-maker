...
- `InMemoryMarketSnapshotRepository` mirrors the PostgreSQL market_snapshots contract：保存 `snapshotId`、`chainId`、`tokenIn`、`tokenOut`、`midPrice`、`liquidityUsd`、`volatilityBps`、non-empty `source`、`observedAt` 和 `createdAt`。Snapshot persistence rejects malformed root payloads and missing `request` / `snapshot` objects before field access or state mutation。同一 `snapshotId` 的完全相同写入是幂等重放，任何 price、liquidity、volatility、pair、source 或 observedAt 改写都会被拒绝。Quote Service 必须先保存 market snapshot，再进入 routing、pricing、quote persistence、risk 或 signer。
