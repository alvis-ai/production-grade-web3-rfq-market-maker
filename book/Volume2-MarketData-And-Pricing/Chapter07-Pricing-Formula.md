...
当前后端 `formula-v4` 实现可测试的实时路径子集：`baseSpreadBps = 8`，internal inventory route buffer 为 2 bps，`marketSpreadBps` 来自不可变行情快照，`volatilityPremiumBps = ceil(volatilityBps / 5)`，`sizeImpactBps = ceil(USD notional * 10000 / expectedLiquidityUsd)` 且最高 250 bps，最终 `totalAdjustmentBps` 被限制在 0 到 2500 bps。USD notional 由 token registry 的 decimals、`usdReference` 和方向化 mid price 以有理数计算。`marketSpreadBps`、`inventorySkewBps`、`volatilityPremiumBps` 和 `hedgeCostBps` 独立进入结果与报价审计记录；它确保以 CEX mid 为 fair reference 时不会把 mid 到 executable bid 的退出成本漏掉。当前 `hedgeCostBps` 是 hedge failure pressure 风险成本，不能与 market spread 或 inventory skew 合并。
