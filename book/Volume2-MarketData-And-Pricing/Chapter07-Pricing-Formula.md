...
当前后端 `formula-v3` 实现可测试的实时路径子集：`baseSpreadBps = 8`，`internalInventoryBufferBps = 2`，`volatilityPremiumBps = ceil(volatilityBps / 5)`，`sizeImpactBps = ceil(USD notional * 10000 / expectedLiquidityUsd)` 且最高 250 bps，最终 `totalAdjustmentBps` 被限制在 0 到 2500 bps。USD notional 由 token registry 的 decimals、`usdReference` 和方向化 mid price 以有理数计算，不再把 `amountIn` base units 直接与美元流动性相除。`inventorySkewBps`、`volatilityPremiumBps` 和 `hedgeCostBps` 独立进入结果与报价审计记录；当前 `hedgeCostBps` 是 hedge failure pressure 风险成本，后续可由外部 venue 深度和手续费模型替换，但不能再次合并进 inventory skew。
...
- `pricingVersion` 使用 `formula-v3:<venue>`，明确区分旧版把 hedge premium 合并进 inventory skew 的归因方式，便于回放报价时定位参数版本和路由场所。
