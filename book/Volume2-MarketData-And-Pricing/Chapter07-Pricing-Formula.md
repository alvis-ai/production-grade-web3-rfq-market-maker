...
当前后端 `formula-v4` 实现可测试的实时路径子集：`baseSpreadBps = 8`，internal inventory route buffer 为 2 bps，`marketSpreadBps` 来自不可变行情快照，`volatilityPremiumBps = ceil(volatilityBps / 5)`，`sizeImpactBps = ceil(USD notional * 10000 / expectedLiquidityUsd)` 且最高 250 bps，最终 `totalAdjustmentBps` 被限制在 0 到 2500 bps。USD notional 由 token registry 的 decimals、`usdReference` 和方向化 mid price 以有理数计算。`marketSpreadBps`、`inventorySkewBps`、`volatilityPremiumBps` 和 `hedgeCostBps` 独立进入结果与报价审计记录；它确保以 CEX mid 为 fair reference 时不会漏掉方向化执行成本：base-to-quote 对比 best bid，quote-to-base 对比 inverse best ask。当前 `hedgeCostBps` 是 hedge failure pressure 风险成本，不能与 market spread 或 inventory skew 合并。
...
- `pricingVersion` 使用 `formula-v4:<venue>`，明确表示 market spread、volatility premium、inventory skew 和 hedge cost 均独立归因，便于回放报价时定位参数版本和路由场所。
...
- CEX 原生 quoteToken 不是 USD reference：启动拒绝配置，避免把非美元 quote depth 当作 `liquidityUsd`；反向 RFQ 由同一原生订单簿自动派生，不需要把交易所 symbol 倒置配置。
