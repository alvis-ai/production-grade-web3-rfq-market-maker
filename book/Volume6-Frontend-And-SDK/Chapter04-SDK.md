...
- `RFQClientError` preserves request correlation even when an upstream response is non-standard: structured RFQ errors keep `ErrorResponse.traceId`, while the client falls back to the `x-trace-id` response header for unknown error bodies or malformed JSON.
