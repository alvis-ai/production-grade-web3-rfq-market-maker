...
- `RFQClient.pnl()` validates successful PnL payloads field by field, including safe identifiers, token addresses, positive uint amount strings, signed gross PnL strings, canonical UTC ISO timestamps generated with `Date.prototype.toISOString()`, `totalTrades === trades.length`, and summary gross PnL matching the trade-level sum.
