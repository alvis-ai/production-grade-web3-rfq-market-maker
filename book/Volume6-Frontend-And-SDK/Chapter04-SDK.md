...
- `RFQClient.getQuote()` validates successful quote status payloads field by field, including required `quoteId`/`status`, optional `snapshotId`, positive `deadline`, 32-byte `txHash`, non-empty settlement/hedge/PnL/error pointers, and lifecycle payload consistency between status and settlement pointers.
