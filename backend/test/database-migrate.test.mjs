...
test("database migration runner adds the Redis quote exposure audit projection", async () => {
  const appliedNames = [
    "base-schema", "settlement-canonical", "hedge-worker-queue", "analytics-outbox",
    "post-trade-reconciliation", "quote-snapshot-pnl", "settlement-indexer", "submit-reservations",
    "risk-notional-reasons", "risk-market-regime-reasons", "open-quote-exposure", "pricing-attribution",
  ...
  console.log = () => {};
  try {
    await migrateUpTo(pool, "037");
  } finally {
    console.log = originalLog;
  }

  assert.equal(client.queries.some(({ sql }) => sql.includes("ADD COLUMN IF NOT EXISTS ledger_expires_at")), true);
  assert.equal(client.queries.some(({ sql }) => sql.includes("CREATE TABLE IF NOT EXISTS quote_exposure_ledger_events")), true);
  assert.equal(client.queries.some(({ sql }) =>
