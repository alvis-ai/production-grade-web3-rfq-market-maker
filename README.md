...
`portfolioVar` adds a portfolio-wide pre-sign budget using the conservative `component-sum-v1` model. Every non-USD risk token must name an explicit USD-reference valuation pair. Production combines an immutable bounded-age inventory view, valuation snapshots, Redis active-quote token deltas and the candidate under one chain lease; it rejects `PORTFOLIO_VAR_LIMIT_EXCEEDED` before signing when `ceil(abs(exposureUsd) * volatilityBps * confidenceMultiplierBps / 10000^2)` exceeds the configured budget. PostgreSQL compatibility mode uses a chain advisory lock plus a short `SHARE` lock, while local memory mode uses a per-chain mutex. Accepted evidence is stored in Redis and mirrored into `var_evaluation` for replay and audit.
