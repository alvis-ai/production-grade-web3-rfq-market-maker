...
The production composition now uses `Redis idempotency -> (prepare || exposure) -> authorize -> signer -> finalize`, with cumulative Redis/Valkey Stream events projected asynchronously into PostgreSQL. The successful quote response no longer queries PostgreSQL for idempotency or issuance. `GET /quote/:id` serves unprojected hot status from Redis, while `POST /submit` waits for the matching finalized projection before entering PostgreSQL post-trade processing. The final concurrency-one run reported p50 19.30 ms / p99 42.19 ms with zero errors; the post-barrier concurrency-five diagnostic reported p50 35.91 ms / p99 54.94 ms and eight fail-closed base-risk errors. Both journal backlogs drained to zero with zero mirror errors. The system therefore still does not meet p50 < 10 ms / p99 < 50 ms / zero errors. ADR-0021 records the current boundary; request-time dynamic toxic-flow and daily-loss PostgreSQL providers plus signer/Redis round trips remain the next latency work.
