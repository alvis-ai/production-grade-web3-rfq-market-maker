...
The production composition now uses process-local market/risk/control generations followed by `Redis idempotency -> (prepare || exposure) -> authorize -> signer -> finalize`, with cumulative Redis/Valkey Stream events projected asynchronously into PostgreSQL. The successful quote response no longer queries PostgreSQL or chain RPC. `GET /quote/:id` serves unprojected hot status from Redis, while `POST /submit` waits for the matching finalized projection before entering PostgreSQL post-trade processing. ADR-0023 replaced the exposure lease with versioned CAS, and ADR-0025 replaced the gateway's global Fetch signer adapter with a bounded native keep-alive pool. Against the rebuilt dependency stack, 10 warmups plus 100 measured concurrency-one quotes returned zero errors, p50 12.72 ms and p99 17.56 ms; average gateway signing was 5.48 ms. The immediately preceding image reported p50 14.88 ms, p99 19.44 ms and 6.41 ms signing, so the transport improvement reached the complete quote path without removing independent EIP-712 recovery or pre-response signer audit. Two clean concurrency-five runs returned zero errors with p50 24.39-26.27 ms, p99 36.84-40.47 ms and signing 6.05-6.42 ms. One separate diagnostic run returned p99 135.07 ms because `issuance_persistence` entered the 250 ms bucket; zero issuance mirror errors/backlog and two clean reruns did not reproduce it, but it remains tail-risk evidence rather than being discarded as a pass. The system therefore still fails p50 < 10 ms and has not proved stable p99 under a production load profile. The remaining common-path budget is dominated by exposure CAS plus serial Redis idempotency/authorization/finalization; the next latency work must reduce those boundaries while preserving atomic admission, nonce ownership and durable audit.
