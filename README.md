...
`portfolioVar` adds a portfolio-wide pre-sign budget using the conservative `component-sum-v1` model. Every non-USD risk token must name an explicit USD-reference valuation pair. Production combines an immutable bounded-age inventory view, valuation snapshots, Redis active-quote token deltas and the candidate under one chain lease; it rejects `PORTFOLIO_VAR_LIMIT_EXCEEDED` before signing when `ceil(abs(exposureUsd) * volatilityBps * confidenceMultiplierBps / 10000^2)` exceeds the configured budget. The common path uses one fused Lua call to acquire the owner-token lease, clean expired entries and return the complete evaluation state, then one fused commit that rechecks exact limits, appends the Stream event, mutates aggregates and conditionally unlocks. PostgreSQL compatibility mode uses a chain advisory lock plus a short `SHARE` lock, while local memory mode uses a per-chain mutex. Accepted evidence is stored in Redis and mirrored into `var_evaluation` for replay and audit.
...
The production composition now uses process-local market/risk/control generations followed by `Redis idempotency -> (prepare || exposure) -> authorize -> signer -> finalize`, with cumulative Redis/Valkey Stream events projected asynchronously into PostgreSQL. The successful quote response no longer queries PostgreSQL or chain RPC. `GET /quote/:id` serves unprojected hot status from Redis, while `POST /submit` waits for the matching finalized projection before entering PostgreSQL post-trade processing. After ADR-0022, a rebuilt dependency-stack run with 10 warmups and 100 measured quotes reported concurrency-one p50 19.50 ms / p99 28.72 ms and concurrency-five p50 32.51 ms / p99 76.24 ms, both with zero errors. The risk stage averaged 0.10-0.11 ms and market/pricing hot reads about 0.01 ms; remote signing averaged 7.45-9.22 ms, while exposure and Redis issuance stages dominated the remaining budget under concurrency. The system therefore still does not meet p50 < 10 ms / p99 < 50 ms / zero errors. ADR-0021 and ADR-0022 record the current boundary; signer and Redis round-trip reduction are the next latency work, without weakening durable authorization or exposure admission.
