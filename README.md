...
Direct `buildServer(options)` embedding follows the same runtime bounds for `bodyLimitBytes` and `quoteTtlSeconds`, and rejects non-boolean `enableHsts` or `trustProxy` values before the Fastify instance is created.
