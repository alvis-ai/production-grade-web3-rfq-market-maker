...
`make verify` runs skeleton, documentation, Docker Compose, EIP-712, ABI, API error-code, database schema, backend, SDK, frontend and local API smoke checks through one entrypoint. If Foundry is installed locally it also runs `make contract-test`; otherwise contract tests remain enforced by the dedicated GitHub Actions contract workflow.
