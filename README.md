...
RFQ_PRICING_CACHE_TTL_MS=100
RFQ_PRICING_CACHE_MAX_ENTRIES=10000
RFQ_HOT_STATE_REFRESH_INTERVAL_MS=250
RFQ_HOT_STATE_MAX_AGE_MS=2000
RFQ_TOXIC_FLOW_HOT_STATE_MAX_ENTRIES=100000
...
RFQ_RATE_LIMIT_LOCAL_PERMIT_BATCH_SIZE=8
RFQ_RATE_LIMIT_MAX_LOCAL_BUCKETS=10000
...
The backend signer uses the same `ProductionGradeRFQ` EIP-712 domain as the SDK and `RFQSettlement` contract. Local development uses `RFQ_SIGNER_MODE=local` inside the isolated Compose signer service, derives the trusted signer from `RFQ_SIGNER_PRIVATE_KEY`, and binds signatures to `RFQ_SETTLEMENT_ADDRESS`. Non-local API runtime uses `RFQ_SIGNER_MODE=remote` over HTTPS, while only the isolated signer workload uses `RFQ_SIGNER_MODE=aws-kms`; both require an explicit `RFQ_TRUSTED_SIGNER_ADDRESS` and reject raw private keys. The settlement verifier always recovers the signature against that explicit signer identity before settlement evidence is consumed, and the remote signer client performs the same recovery before returning a quote. `HOST` defaults to `127.0.0.1` and must not contain whitespace; `PORT` defaults to `3000` and must be a base-10 integer from 1 to 65535. Backend startup reads only own environment fields, so prototype-backed `HOST`, `PORT`, `RFQ_SHUTDOWN_TIMEOUT_MS`, `RFQ_QUOTE_TTL_SECONDS`, signer, CORS, HSTS, proxy, Redis, token-registry or risk-policy values are treated as unset. `RFQ_SHUTDOWN_TIMEOUT_MS` defaults to 20000 and accepts only canonical base-10 integers from 1000 through 120000; the first termination signal starts bounded cleanup, while timeout or a second signal exits 1. Kubernetes keeps this at 20 seconds inside a reviewed 5-second `preStop` and 30-second termination window. `RFQ_QUOTE_TTL_SECONDS` controls the signed quote lifetime and must be a base-10 integer from 1 to 3600; keep it short enough to limit stale price execution. `RFQ_PRICING_CACHE_TTL_MS` defaults to 100 and accepts 1 through 5000; `RFQ_PRICING_CACHE_MAX_ENTRIES` defaults to 10000 and accepts 1 through 1000000. They bound an exact, process-local cache around the default formula pricing engine. Cache keys include request, market snapshot, route, inventory skew and hedge-cost state; signed quotes, nonces, risk decisions and exposure reservations are never cached. Custom pricing engines are not wrapped implicitly. `RFQ_HOT_STATE_REFRESH_INTERVAL_MS` defaults to 250 and controls background refresh for quote control, toxic-flow scores, hedge penalties, daily-loss evidence, USD-reference health and settlement-indexer evidence. `RFQ_HOT_STATE_MAX_AGE_MS` defaults to 2000, must cover at least two refresh intervals and fails quote/readiness checks closed once the last complete immutable generation expires. `RFQ_TOXIC_FLOW_HOT_STATE_MAX_ENTRIES` defaults to 100000 and rejects startup/refresh when PostgreSQL contains more scores than the reviewed in-memory capacity. These snapshots remove PostgreSQL and RPC reads from the default quote risk path; PostgreSQL remains the update/audit source, while Redis/Valkey remains the distributed issuance, rate-limit and exposure authority. `RFQ_BODY_LIMIT_BYTES` controls the maximum JSON request body size and must be a base-10 integer from 1024 to 1048576. `RFQ_CORS_ALLOWED_ORIGINS` is a comma-separated allowlist of HTTP(S) URL origins that may call the API; startup rejects entries with paths, query strings, fragments, credentials, or wildcards, then normalizes each accepted origin with `URL.origin`. `RFQ_ENABLE_HSTS` must only be enabled when the public API is served through HTTPS. `RFQ_TRUST_PROXY` defaults to `false`; only enable it when a trusted reverse proxy or ingress strips untrusted `x-forwarded-for` input and writes the client IP. `RFQ_TOKEN_REGISTRY_JSON` is the trusted, startup-validated source for each supported chain/token address, symbol, decimals, whitelist state and USD-reference status. `RFQ_RISK_POLICY_JSON` defines chain/token exposure, portfolio VaR/Delta and market-regime limits. Its required `gammaGuardrail` computes bounded utilization for both projected inventory legs, every USD-reference notional leg and current volatility with integer ceiling; `piecewise-convexity-v1` maps each dimension to `0/2500/5000` bps premium and rejects with `GAMMA_GUARDRAIL_TRIGGERED` when the combined multiplier reaches `maxRiskMultiplierBps`. Missing inventory projection fails closed as `RISK_ENGINE_UNAVAILABLE`. Outside local environments, `RFQ_DAILY_LOSS_CONFIG_JSON` and PostgreSQL are mandatory for the default risk engine, but the database is consumed only by startup/background refresh rather than each quote. Every managed USD-reference token needs a positive `maxLossUsdE18`; the guard sums only completed hedge-net PnL in the current UTC day, rejects at the exact negative boundary with `DAILY_LOSS_LIMIT_EXCEEDED`, degrades readiness after the budget is exhausted, and fails closed when accounting evidence is unavailable or malformed. `RFQ_USD_REFERENCE_CONFIG_JSON` is also mandatory and must provide a reviewed Chainlink token/USD feed for every managed token marked `usdReference`; startup rejects missing coverage or symbol/feed identity mismatch. The oracle guard validates RPC chain identity, feed metadata, L2 sequencer state, round completeness, timestamps and circuit-breaker bounds in the background before comparing the answer with the 1 USD peg. A confirmed deviation above `maxDeviationBps` records `USD_REFERENCE_DEPEG`, degrades readiness and blocks Signer; unavailable, malformed or stale hot oracle evidence fails closed as `RISK_ENGINE_UNAVAILABLE`. The exact feed round and daily accounting window are hashed into `risk_decisions.policy_version` for replay correlation. `VITE_RFQ_API_BASE_URL` configures the browser API endpoint and must be an absolute HTTP(S) URL with an optional path prefix, no credentials, no wildcard host, and no query string or fragment. `VITE_RFQ_SETTLEMENT_ADDRESS` configures the browser-side `RFQSettlement.submitQuote` target. `VITE_WALLETCONNECT_PROJECT_ID` configures RainbowKit wallet connection and must be a 128-character-or-shorter safe string containing only letters, numbers, underscore, or hyphen.

The remote signer client uses a native HTTP(S) keep-alive pool instead of the global Fetch/Web Streams adapter. `RFQ_SIGNER_SERVICE_MAX_CONNECTIONS` defaults to 32 and accepts 1 through 256; it bounds active and idle signer sockets per API process, while shutdown destroys the pool. TLS still uses Node's hostname and certificate validation, including the mounted CA from `NODE_EXTRA_CA_CERTS`. The client keeps connection, response streaming and JSON decoding inside `RFQ_SIGNER_SERVICE_REQUEST_TIMEOUT_MS`. Successful `/internal/sign` and `/ready` bodies are streamed through a 1 KiB cap before decoding; an oversized or stalled signer cannot force the API to buffer an unbounded payload or hold a quote request open after headers. Non-success bodies are destroyed without parsing, and every transport, status, payload or recovered-signer mismatch remains the same generic `SIGNER_UNAVAILABLE` response. Injected Fetch implementations remain available only as a deterministic adapter-test boundary; the default runtime uses the bounded native pool.

Production enables `RFQ_SIGNER_ATOMIC_QUOTE_COMMIT=true` on both API and isolated signer. After base risk and exposure reservation, one Redis admission transaction persists cumulative snapshot/request/route/risk state, binds idempotency and appends the `authorized` event; the production path does not emit a separate `prepared` event. Redis authorization stores a SHA-256 commitment over the exact unsigned quote, principal, snapshot, pricing decomposition, policy and idempotency ownership; the signer verifies it from hot state before KMS/HSM access. After signing, one hash-tagged Lua transaction finalizes the quote, completes idempotency, indexes the nonce and appends both issuance and signer-audit events before returning a finalization hash. The API still independently recovers the EIP-712 signer and recomputes that hash. Lost signer responses recover from Redis; exposure is released only when Redis proves no finalization exists. `RFQ_SIGNER_AUDIT_STREAM_KEY` must share the issuance ledger hash tag, and destructive stream recreation requires a new `RFQ_SIGNER_AUDIT_STREAM_EPOCH` plus a paused, coordinated signer/API capability rollout. `make quote-issuance-integration-check` validates two cumulative issuance events, concurrent admission replay and the real Redis/PostgreSQL path.
...
`maxUserOpenNotionalUsd` and `maxPairOpenNotionalUsd` bound cumulative activity across signed quotes that remain executable. Before signing, Quote Service converts the trusted USD-reference side to an exact 18-decimal USD integer. Production uses `RFQ_QUOTE_EXPOSURE_BACKEND=redis-stream`: one hash-tagged Redis/Valkey ledger atomically checks user, pair and Treasury-output totals, updates exact decimal-string aggregates and token deltas, and appends a versioned stream event. Portfolio VaR reads immutable process-local inventory and valuation snapshots; it does not query PostgreSQL on the exposure stage. Reservations remain active through a bounded grace after quote expiry so an inventory refresh cannot briefly omit both an expiring quote and a newly settled position.

PostgreSQL remains the long-term audit and recovery projection. A consumer group inserts each `<ledger-epoch>:<stream-id>` event and applies only a newer per-quote stream position before `XACK`/`XDEL`. Production requires `rediss://`, healthy AOF, at least one `WAIT` replica acknowledgement, an explicit pre-bootstrapped epoch and bounded backlog. It never falls back automatically to PostgreSQL because two concurrent admission authorities could oversubscribe risk. The PostgreSQL implementation remains a local/test rollback backend only.

Initialize a new production epoch only while quote admission is stopped and the reviewed prefix is empty. Set `RFQ_QUOTE_EXPOSURE_REDIS_URL`, `RFQ_QUOTE_EXPOSURE_KEY_PREFIX`, `RFQ_QUOTE_EXPOSURE_LEDGER_EPOCH` and `RFQ_QUOTE_EXPOSURE_BOOTSTRAP_CONFIRM=initialize-empty-quote-exposure-ledger`, then run `make quote-exposure-ledger-bootstrap`. The command requires healthy AOF and refuses an existing key. `RFQ_QUOTE_EXPOSURE_MIRROR_CLEANUP_INTERVAL_MS` defaults to 10000 ms; a mirror or cleanup failure blocks new reserves in memory while release remains available.

Backend CI proves both authorities against real dependencies rather than storage mocks alone. The PostgreSQL compatibility check creates isolated `requested` quotes, races two database sessions against user, pair and Treasury capacity, requires exactly one winner, verifies idempotent replay and release, then removes every fixture. The production-path checks additionally use Redis plus PostgreSQL mirror. To reproduce the compatibility check after migrations:
...
The Redis/Valkey checks additionally cover exact integers beyond IEEE-754, atomic Treasury limits, release recovery, stream mirroring and append-only PostgreSQL events:

```bash
RFQ_QUOTE_EXPOSURE_REDIS_URL='redis://127.0.0.1:6379/0' \
RFQ_QUOTE_EXPOSURE_REDIS_INTEGRATION_CONFIRM=yes \
node scripts/quote-exposure-redis-integration-check.mjs

DATABASE_URL='postgres://rfq:rfq@127.0.0.1:5432/rfq_market_maker' \
RFQ_QUOTE_EXPOSURE_REDIS_URL='redis://127.0.0.1:6379/0' \
RFQ_QUOTE_EXPOSURE_LEDGER_INTEGRATION_CONFIRM=yes \
make quote-exposure-ledger-integration-check
```

`portfolioVar` adds a portfolio-wide pre-sign budget using the conservative `component-sum-v1` model. Every non-USD risk token must name an explicit USD-reference valuation pair. Production combines an immutable bounded-age inventory view, valuation snapshots, Redis active-quote token deltas and the candidate from one versioned chain snapshot; it rejects `PORTFOLIO_VAR_LIMIT_EXCEEDED` before signing when `ceil(abs(exposureUsd) * volatilityBps * confidenceMultiplierBps / 10000^2)` exceeds the configured budget. The common path uses one Lua read to clean expired entries and return the chain generation plus complete evaluation state, then a compare-and-set commit that rechecks the generation and exact user, pair and Treasury limits before mutating aggregates and appending the Stream event. A generation conflict reruns in-memory VaR/Delta against the new snapshot within a bounded retry budget; release keeps the owner-token lease because it does not perform in-memory evaluation. PostgreSQL compatibility mode uses a chain advisory lock plus a short `SHARE` lock, while local memory mode uses a per-chain mutex. Accepted evidence is stored in Redis and mirrored into `var_evaluation` for replay and audit.
...
The same pre-sign reservation also protects actual Treasury output capacity in receipt-confirmed runtimes. The default production runtime uses the chain RPC declared by `RFQ_RECEIPT_CONFIG_JSON` to verify `eth_chainId` and read `RFQSettlement.treasury()` plus each managed token balance at one block per target, but performs those calls during startup warmup and a background refresh rather than inside `/quote`. A refresh publishes one immutable generation only after every configured chain/token target validates; partial or malformed generations are discarded. Request handling reads that process-local generation, rejects missing or older-than-configured state, and passes its balance evidence to the Redis/Valkey exposure CAS. The commit compares the evaluated chain generation and `(chainId, tokenOut)` reservations against current Redis state while atomically updating user, pair, Treasury, VaR and Delta aggregates and appending the audit event; a conflict returns without mutation and triggers a bounded full reevaluation. `RFQ_TREASURY_LIQUIDITY_REFRESH_INTERVAL_MS` defaults to 100 ms, `RFQ_TREASURY_LIQUIDITY_MAX_AGE_MS` defaults to 1000 ms and must cover at least two refresh intervals, and `RFQ_QUOTE_EXPOSURE_EXPIRY_GRACE_SECONDS` must be strictly longer than the maximum hot-state age. A quote that would oversubscribe the asset is rejected internally as `TREASURY_LIQUIDITY_INSUFFICIENT` before KMS signing. Failed refreshes retain the previous generation only until its freshness bound; stale state, wrong-chain RPC, malformed contract data, Redis durability failure or mirror failure degrades risk readiness and blocks new signatures. PostgreSQL remains the audit projection and is not queried for Treasury admission on the production quote path.
...
`POST /quote` accepts a principal-scoped `Idempotency-Key` containing 16 to 128 safe ASCII characters and requires it outside local environments. Reusing the same key with the same normalized payload returns the exact signed response; changing the payload returns `IDEMPOTENCY_KEY_CONFLICT`, while an active owner lease returns `IDEMPOTENCY_REQUEST_IN_PROGRESS`. Production uses the Redis/Valkey issuance ledger to atomically bind the key to `quote_id` in the cumulative authorization admission, then enforce nonce uniqueness, finalize the response and append the terminal cumulative audit event before returning. PostgreSQL receives the complete state from the `authorized` and `finalized` events through an ordered asynchronous projection. The explicit local compatibility backend retains synchronous PostgreSQL issuance. `RFQ_QUOTE_IDEMPOTENCY_LEASE_MS` defaults to at least `60000`, must be between `10000` and `3900000`, and must exceed the signed quote TTL.
...
Leave `RFQ_TRUST_PROXY=false`; only enable it when a trusted reverse proxy or ingress strips untrusted `x-forwarded-for` headers and sets the canonical client address. When enabled, the rate limiter keys by the first `x-forwarded-for` entry after enforcing the 128 character limit and `[A-Za-z0-9_.:-]` character set; otherwise it uses the direct socket IP. Redis atomically reserves a bounded permit batch against the fixed-window global count, then the owning process consumes that lease from a bounded monotonic-TTL cache. Unused permits are never reissued inside the window, so eviction or process death can reduce availability but cannot exceed the global limit. `RFQ_RATE_LIMIT_LOCAL_PERMIT_BATCH_SIZE` and `RFQ_RATE_LIMIT_MAX_LOCAL_BUCKETS` bound the latency/fairness and memory tradeoffs. Redis errors return `RATE_LIMIT_UNAVAILABLE`/503 and degrade `rateLimitStore` readiness; the gateway never silently fails open.
...
API, hedge, analytics, reconciliation, settlement-indexer, and toxic-flow analyzer Kubernetes Deployments run the migration entrypoint in an init container using `migrationSecret`. Migration discovery and execution are serialized by a PostgreSQL session advisory lock, allowing multiple rollout pods to start safely while ensuring all migrations through `038-quote-issuance-journal.sql` are committed before any process checks readiness. A competing init container retries the session lock for at most four minutes; the CLI logs `connecting`、`acquiring-lock`、`reading-state` and each applied version, always closes its pool on completion or failure, and enforces a five-minute hard deadline by default. `RFQ_MIGRATION_TIMEOUT_MS` can set that deadline from 1000 to 1800000 ms; keep it above the expected schema-change duration and below the platform's init-container restart budget. Before applying migration 017 to an existing deployment, stop quote admission and wait at least the maximum quote TTL; historical rows receive isolated legacy principals and are intentionally unavailable through institutional APIs. The DDL-capable migrator credential is not mounted into runtime containers. The isolated signer uses separate verify-full `RFQ_SIGNER_AUDIT_DATABASE_URL` and `rediss://` `RFQ_SIGNER_AUDIT_REDIS_URL` credentials. Its PostgreSQL role only has `INSERT` on `signer_audit_events` and `USAGE` on its sequence; migration 028 binds every new signing audit row to risk authorization context, migration 036 makes signer stream replay idempotent through `source_stream_id`, migration 037 adds the append-only quote-exposure projection, and migration 038 adds the cumulative quote-issuance journal plus ordered projection version. Durable stream admission completes before a signature is returned; PostgreSQL mirroring is ordered before stream acknowledgement, and exposure projection waits for the matching prepared quote projection. Production operators must provision `rfq.analytics.v1` before starting analytics consumers; auto topic creation is intentionally disabled.
...
`make benchmark-quote-http` runs the same default 10 warmup plus 100 measured requests against `RFQ_HTTP_BENCHMARK_API_URL` (default `http://127.0.0.1:3000`). The Compose backend exercises real HTTP, PostgreSQL, Redis and the isolated signer service; set `RFQ_BACKEND_HOST_PORT` when host port 3000 is unavailable, and point the benchmark URL at that port. The benchmark takes `/metrics` snapshots around the measured window and reports exact pricing-cache hit/miss deltas plus average and p99 bucket upper bounds for every observed quote dependency stage. It enforces p50 <= 10 ms, p99 <= 50 ms and zero errors by default. Use `RFQ_HTTP_BENCHMARK_API_KEY` for an authenticated target and `RFQ_HTTP_BENCHMARK_QUOTE_JSON` for its reviewed pair/amount; non-loopback targets must use HTTPS. Workload controls are `RFQ_HTTP_BENCHMARK_QUOTE_REQUESTS`, `RFQ_HTTP_BENCHMARK_QUOTE_WARMUP_REQUESTS`, `RFQ_HTTP_BENCHMARK_QUOTE_CONCURRENCY` and `RFQ_HTTP_BENCHMARK_REQUEST_TIMEOUT_MS`; SLO controls are `RFQ_HTTP_BENCHMARK_MAX_P50_MS`, `RFQ_HTTP_BENCHMARK_MAX_P99_MS`, `RFQ_HTTP_BENCHMARK_MAX_ERRORS` and `RFQ_HTTP_BENCHMARK_ENFORCE_SLO`. Set enforcement to `false` only for diagnosis, and preserve the reported violations. The default workload consumes 110 quote requests, so run it against an idle rate-limit window or a separately reviewed benchmark principal. The signer service separately exports fixed `validation|digest|authorization|signature|audit` stage histograms; compare them with the gateway `signing` stage to isolate signer computation from transport, JSON and gateway recovery verification.

With atomic Redis quote admission enabled, a clean rebuilt Compose window at concurrency 1 completed 10 warmups plus 100 measured quotes with zero errors, 80.83 requests/second, p50 11.95 ms and p99 16.51 ms. The preceding SHA-cached baseline was p50 13.00 ms and p99 18.55 ms. The separate `quote_preparation_persistence` stage has zero production samples; `authorization_persistence` measures the combined admission. This is an improvement but still fails the p50 <= 10 ms target.

The production composition now uses process-local market/risk/control generations followed by `Redis idempotency -> (prepare || exposure) -> authorize -> signer atomic audit/finalization`, with cumulative Redis/Valkey Stream events projected asynchronously into PostgreSQL. The successful quote response does not query PostgreSQL, chain RPC or CEX REST. `GET /quote/:id` serves unprojected hot status from Redis, while `POST /submit` waits for the finalized projection before PostgreSQL post-trade processing. The signer independently verifies a canonical full-quote authorization before key access, then one Lua transaction commits signer audit, quote finalization, idempotency, nonce and issuance evidence. The rebuilt dependency stack returned zero errors for 10 warmups plus 100 serial samples with p50 12.96 ms, p99 21.13 ms and 73.21 requests/second, improving the prior p50 13.37 ms and p99 31.71 ms baseline. Signing averaged 5.19 ms, exposure 2.40 ms, preparation 1.38 ms, authorization 1.37 ms and idempotency 1.00 ms; successful gateway finalization samples were eliminated. A measured authorization/signing overlap experiment regressed to p50 13.10 ms and p99 53.23 ms and was reverted. The system therefore passes the observed p99 target in this local window but still fails p50 < 10 ms and has not proved production KMS/TLS/cross-zone SLOs. Further work must atomically fuse serial hot-state admission boundaries rather than add polling races or optimize routing, pricing, rate limiting and quote-control reads that are already sub-millisecond.

The Redis transaction stores now use SHA-cached scripts with exact `NOSCRIPT` fallback and monotonic cross-process audit timestamps. A like-for-like rebuilt run returned zero errors with p50 13.00 ms, p99 18.55 ms and 75.15 requests/second; exposure averaged 2.21 ms, but authorization and signing remained 1.38 ms and 5.23 ms. This removes repeated Lua source transfer and improved the observed tail, but did not improve p50. The next latency step must reduce durable state transitions rather than tune script transport.
