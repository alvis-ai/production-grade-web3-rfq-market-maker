...
`RFQClientError` preserves structured API errors. For HTTP 429 `RATE_LIMITED` responses, the SDK exposes `retryAfterSeconds` from the `Retry-After` header so callers can back off without parsing headers directly. Successful quote, submit, quote status, hedge, and PnL responses are validated field by field, including identifiers, signatures, token addresses, uint/int amount strings, timestamps, `totalTrades`, and aggregate gross PnL consistency.
