...
The remote signer client keeps connection, response streaming and JSON decoding inside `RFQ_SIGNER_SERVICE_REQUEST_TIMEOUT_MS`. Successful `/internal/sign` and `/ready` bodies are streamed through a 1 KiB cap before decoding; an oversized or stalled signer cannot force the API to buffer an unbounded payload or hold a quote request open after headers. Non-success bodies are canceled without parsing, and every transport, status, payload or recovered-signer mismatch remains the same generic `SIGNER_UNAVAILABLE` response.
...
The production composition now uses process-local market/risk/control generations followed by `Redis idempotency -> (prepare || exposure) -> authorize -> signer -> finalize`, with cumulative Redis/Valkey Stream events projected asynchronously into PostgreSQL. The successful quote response no longer queries PostgreSQL or chain RPC. `GET /quote/:id` serves unprojected hot status from Redis, while `POST /submit` waits for the matching finalized projection before entering PostgreSQL post-trade processing. After ADR-0023 replaced the reserve lease with a versioned read/evaluate/CAS loop, the final rebuilt dependency-stack image with 10 warmups and 100 measured quotes reported concurrency-one p50 14.96 ms / p99 22.78 ms with zero errors and 1.87 ms average exposure. Three clean concurrency-five runs reported p50 25.81-28.01 ms, p99 41.74-46.83 ms, exposure 8.15-8.77 ms and remote signing 7.60-8.50 ms, all with zero errors. The final concurrency-five run resolved 79 generation conflicts without retry exhaustion. The pre-change concurrency-five baseline was p50 32.51 ms / p99 76.24 ms with 9.35 ms average exposure. The system therefore now meets the measured p99 and zero-error bounds in these runs, but still fails p50 < 10 ms. A fresh concurrency-one run reported p50 14.88 ms, p99 19.44 ms and 6.41 ms average gateway signing latency. Across its 110 signer calls, isolated signer averages were validation 0.046 ms, digest 0.272 ms, signature 0.881 ms and durable audit admission 1.096 ms. The roughly 4.1 ms remainder belongs to the service transport/JSON boundary plus mandatory gateway recovery verification. The essential serial stage averages already total about 10.94 ms before API overhead, so micro-optimizing signer digest computation cannot satisfy the p50 objective. `rfq_quote_exposure_ledger_version_conflicts_total`, coordination-wait histograms and fixed signer-stage histograms now expose the remaining pressure; the next latency change requires a reviewed transport or authorization-protocol decision rather than removing audit durability or signature verification.
