...
assertContains(redisRateLimiterSource, [
  ...
  "if current >= limit",
  "math.min(requested, limit - current)",
  "assertScriptResult(result)",
  "normalizeRateLimitInput(input)",
  "limitForRateLimitEndpoint",
  "consumeLocalPermit(key)",
  "allocatePermits(key, limit)",
  "localPermitBatchSize",
  "maxLocalBuckets",
  "this.allocations.get(key)",
  "async checkHealth()",
  'response !== "PONG"',
  "lazyConnect: true",
...
assertContains(mainSource, [
  ...
  "rateLimit cannot be disabled when NODE_ENV=${nodeEnv}",
  ...
  "enforceRateLimit(",
  'observeQuoteBoundaryStage(metricsService, "rate_limit"',
  '"quote",',
  '"submit",\n        request,',
  '"status",\n        request,',
  ...
  "clientIdForRateLimit(request, trustProxy, principal)",
...
assertContains(apiRedisRateLimitTestSource, [
  ...
  "RFQ API permits disabled rate limiting only in local environments",
...
assertContains(gatewayChapterSource, [
  ...
  "拒绝 `buildServer({ rateLimit: false })`",
  "单个 Lua script",
  "预留不超过剩余全局容量的有界 permit batch",
  "进程崩溃或 eviction 只会浪费未用 permit",
  "`x-ratelimit-remaining` 是保守的本地 lease 余量",
...
assertContains(readmeSource, [
  ...
  "rejects the embedded `buildServer({ rateLimit: false })` escape hatch",
  "requires `RFQ_REDIS_URL` to use `rediss://`",
  "Redis atomically reserves a bounded permit batch",
  "Unused permits are never reissued inside the window",
  "`RFQ_RATE_LIMIT_LOCAL_PERMIT_BATCH_SIZE`",
  "`RFQ_RATE_LIMIT_MAX_LOCAL_BUCKETS`",
  "`RATE_LIMIT_UNAVAILABLE`/503",
], "README.md");

assertContains(envExampleSource, [
