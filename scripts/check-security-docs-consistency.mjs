...
const requiredTerms = {
  threatModel: [
    ...
    "Owner and role authority divergence",
    "Quote replay",
    ...
    "Oversized, silent or regressed CEX market data",
    "Risk bypass",
    "Mempool MEV",
    "Event duplication",
    "Chain reorg",
  ...
  auditChecklist: [
    ...
    "A Settlement owner without DEFAULT_ADMIN_ROLE cannot transfer ownership",
    "Treasury, Settlement and newly whitelisted token configuration rejects EOAs",
    ...
    "Non-local API startup rejects the benchmark-only `rateLimit:false` option",
    ...
    "Non-local API, worker and migration processes require hostname-verified PostgreSQL TLS",
...
const implementedAuditControls = [
  ...
  "Pausable can stop settlement during incident response.",
  "AccessControl protects signer and token whitelist updates.",
  "A Settlement owner without DEFAULT_ADMIN_ROLE cannot transfer ownership to restore administrative roles.",
  "Treasury, Settlement and newly whitelisted token configuration rejects EOAs, and Treasury rotation requires the candidate Treasury to point back to the active Settlement.",
  ...
  "`/quote` validates address format and amount strings.",
  "Risk Engine runs before Signer Service.",
  "Non-local API startup rejects the benchmark-only `rateLimit:false` option, so embedded startup cannot disable the mandatory limiter.",
  "Signer Service cannot be called directly from public API.",
